...
// ─── SKILL MAP TAB COMPONENT ──────────────────────────────────────────────────
const SM_TRACK_CONFIG = {
  aptitude: { label: 'Aptitude', color: '#F59E0B', rgb: '245,158,11' },
  analyst:  { label: 'Analyst',  color: '#3B82F6', rgb: '59,130,246' },
  project:  { label: 'Project',  color: '#10B981', rgb: '16,185,129' },
  dsa:      { label: 'DSA',      color: '#8B5CF6', rgb: '139,92,246' },
  fullstack:{ label: 'Full Stack',color:'#EC4899', rgb: '236,72,153' },
  cscore:   { label: 'CS Core',  color: '#EF4444', rgb: '239,68,68'  },
  embedded: { label: 'Embedded/Robotics', color: '#06B6D4', rgb: '6,182,212' },
  cybersecurity: { label: 'Cybersecurity', color: '#F43F5E', rgb: '244,63,94' },
  devops:   { label: 'DevOps/Cloud', color: '#6366F1', rgb: '99,102,241' },
  uiux:     { label: 'UI/UX Design', color: '#D946EF', rgb: '217,70,239' },
  aiml:     { label: 'AI/ML', color: '#14B8A6', rgb: '20,184,166' },
  personal: { label: 'Creative/Writing', color: '#F4A72A', rgb: '244,167,42' }
};
...
const SM_GRID_DATA = {
  JUNE: {
    ...
    cscore: "OOPS",
    embedded: "C Programming (Pointers, Memory, bitwise)",
    cybersecurity: "Linux & Networking Foundations",
    devops: "Linux + Networking (Shell scripting)",
    uiux: "Design Basics (Figma, Typography, Layout)",
    aiml: "Python & Data Foundations (OOP, NumPy, Pandas)",
  ...
  JULY: {
    ...
    cscore: "DBMS",
    embedded: "Digital Electronics & Computer Arch (Logic gates, FSMs)",
    cybersecurity: "Web Security (OWASP Top 10, SQLi, XSS)",
    devops: "Git & CI/CD (GitHub Actions)",
    uiux: "UX Basics (User Flows, Wireframes, IA)",
    aiml: "Stats & ML Foundations (Regressions, KNN)",
  ...
  OCTOBER: {
    ...
    cscore: "LLD",
    embedded: "RTOS & Advanced Networking (FreeRTOS tasks, Semaphores)",
    cybersecurity: "Cloud Security (AWS IAM, S3, Secrets Mgmt)",
    devops: "Kubernetes (Pods, Deployments, Ingress)",
    uiux: "Responsive Design (Mobile + Desktop)",
    aiml: "NLP & Generative AI (Embeddings, Vector Search, LLM APIs)",
  ...
  NOVEMBER: {
    ...
    cscore: "HLD",
    embedded: "Specialization (Verilog/FPGA, ROS, or Computer Vision)",
    cybersecurity: "Certifications (Security+, Google Cert)",
    devops: "Monitoring & Logging (Prometheus, Grafana)",
    uiux: "Portfolio Curation (Musicverse, CineHub)",
    aiml: "RAG & Agentic AI (Vector DBs, LangChain/LangGraph)",
  ...
  DECEMBER: {
    ...
    embedded: "Systems Portfolio & Integration",
    cybersecurity: "Final Audit Projects & Dashboard",
    devops: "Complete CI/CD & Cloud Launch",
    uiux: "Design Presentation & Launch",
    aiml: "RAG MVP & Campus Placement Assistant",
    personal: "Interview Ready (Creative Curation)"
  }
};

const SM_ROLE_REQS = {
  'Data Analyst':        { color:'#3B82F6', must:['SQL','Python/Pandas','Data Visualization','Excel/Sheets','Statistical Analysis','Dashboard tools (Tableau/Power BI)'], gth:['dbt','BigQuery/Snowflake','A/B Testing','Cohort Analysis'] },
  'Product Analyst':     { color:'#8B5CF6', must:['SQL','Product Metrics (DAU/MAU/Retention)','A/B Testing','Python or Excel','User Journey Mapping','Funnel Analysis'], gth:['Amplitude/Mixpanel','Figma','Market Research','PRD writing'] },
  'Business Analyst':    { color:'#10B981', must:['SQL','Excel','Business Problem Framing','Stakeholder Communication','Data Storytelling','Process Mapping'], gth:['Power BI','Python','Financial Modeling basics','Case Study frameworks'] },
  'Growth Analyst':      { color:'#F59E0B', must:['SEO','Content Strategy','Google Analytics','Social Media Analytics','A/B Testing (copy)','Excel/Sheets'], gth:['HubSpot/similar','Email marketing tools','Canva/Figma','Cohort tracking'] },
  'Sports Analyst':      { color:'#EF4444', must:['Python/Pandas','Sports Domain Knowledge (IPL/PKL)','Data Visualization','Statistical Thinking','SQL'], gth:['R','Tableau/Power BI','Web Scraping (BeautifulSoup)','Expected Value modeling'] },
  'Financial Analyst':   { color:'#6366F1', must:['Excel (Advanced)','Financial Modeling','Accounting basics','Python (yfinance)','SQL','Data Interpretation'], gth:['Bloomberg Terminal familiarity','DCF','Ratio Analysis','Power BI'] },
  'Content Strategist':  { color:'#EC4899', must:['Content Strategy','Google Analytics (GA4)','HubSpot Marketing Automation','SEO','Content Calendar Management','Brand Messaging'], gth:['HubSpot Marketing','SEMrush','A/B Testing','Audience Research & Campaign Planning'] },
  'SEO Analyst':         { color:'#14B8A6', must:['SEO','Google Analytics','Keyword Research','On-page Optimization','Technical SEO'], gth:['SEMrush/Ahrefs','Search Console','Content Strategy','Link Building'] },
  'Full Stack Developer':{ color:'#2ECC71', must:['React','TypeScript','Node.js','Express','MongoDB','HTML/CSS','Vite','Git/GitHub','Vercel'], gth:['Firebase','REST APIs','Tailwind CSS','Framer Motion','JWT/Auth'] },
  'Backend Developer':   { color:'#38BDF8', must:['Node.js','Express','MongoDB','SQL','DBMS','Database normalization','Firebase Admin','REST APIs'], gth:['JWT/Auth','Mongoose','Postman','Error Handling','Query Optimization'] },
  'SDE (Intern)':        { color:'#6366F1', must:['Complexity Analysis','OOPs','Binary Search','Sorting Algorithms','Two Pointers','Sliding Window','Recursion'], gth:['Trees','Advanced Trees/Trie','Backtracking','Divide and Conquer','TypeScript'] },
  'Database Administrator':{ color:'#10B981', must:['SQL','DBMS','Database normalization','Firestore/NoSQL Schema','Query Optimization','Relational Databases','Star Schema'], gth:['Indexes','Firebase Auth','PostgreSQL/SQLite','Data warehousing'] },
  'Content Writer':      { color:'#EC4899', must:['SEO Writing','Content Strategy','Keyword Research','Blog writing','Brand Messaging & Copywriting'], gth:['Medium','LinkedIn publishing','Cinepedia columns','Research writing'] },
  'Copywriter':          { color:'#F59E0B', must:['Brand Messaging & Copywriting','A/B Testing (copy)','Google Ads Search','Google Analytics','HubSpot Marketing Automation'], gth:['Ad Copy','Social hooks','Figma/Canva basics'] },
  'Narrative Designer':  { color:'#A855F7', must:['Brand Messaging & Copywriting','Content Strategy','SEO Writing','Creative Writing','Lore Timelines','Character Bibles'], gth:['Storyboards','Interactive story flow','Creative writing','Nostalgia triggers'] },
  'Growth Marketer':     { color:'#FB923C', must:['Google Analytics','HubSpot Marketing Automation','A/B Testing','SEO','Customer Acquisition Cost (CAC)','Social Media Analytics'], gth:['Email marketing','Conversion rate optimization (CRO)','Google Ads Search','Cohort retention'] }
};
...
function smGetDetails(n) {
  ...
  if(x.includes('sql')||x.includes('dbms')||x.includes('normalization')||x.includes('relational')||x.includes('schema')||x.includes('postgresql'))
    return { category:'Database & Backend Data', impact:'Crucial for designing efficient normalized tables and retrieving data at scale.', pros:['Guarantees data consistency (ACID)','Industry standard for structured enterprise data'], cons:['Schema changes can be difficult','Needs query optimization for large datasets'] };
  if(x.includes('python')||x.includes('pandas')||x.includes('seaborn')||x.includes('eda')||x.includes('statistics')||x.includes('visualization')||x.includes('power bi')||x.includes('excel')||x.includes('kpi')||x.includes('funnel')||x.includes('ltv/cac')||x.includes('metrics')||x.includes('predictive')||x.includes('forecasting')||x.includes('modeling'))
    return { category:'Analytics & Business Intelligence', impact:'Enables translating raw business metrics into actionable growth and product insights.', pros:['Drives data-backed product decisions','Highly visible to stakeholders'], cons:['Correlation does not imply causation','Dirty data leads to misleading conclusions'] };
  if(x.includes('react')||x.includes('typescript')||x.includes('html')||x.includes('css')||x.includes('vite')||x.includes('vercel')||x.includes('javascript')||x.includes('dom'))
    return { category:'Frontend Development', impact:'Critical for building responsive high-performance web applications.', pros:['Rich ecosystem of libraries','Component reuse speeds development'], cons:['Fast-paced ecosystem requires constant learning','JS client-side bugs can break UI'] };
  if(x.includes('node')||x.includes('express')||x.includes('mongodb')||x.includes('git')||x.includes('firebase')||x.includes('api')||x.includes('network'))
    return { category:'Backend & Infrastructure', impact:'Powers server-side logic, API routing, database connections and hosting pipelines.', pros:['Async I/O handles high concurrent traffic','NoSQL offers flexible schema-less structures'], cons:['Debugging async code can be complex','NoSQL lacks standard ACID constraints'] };
  if(x.includes('complexity')||x.includes('oops')||x.includes('search')||x.includes('sorting')||x.includes('pointer')||x.includes('window')||x.includes('recursion')||x.includes('algorithm')||x.includes('lld')||x.includes('hld')||x.includes('tree')||x.includes('trie')||x.includes('backtracking')||x.includes('divide'))
...
const SM_GRID_DETAILS = {
  "june-aptitude": {
    why: "Critical initial filter to clear fast-track screening rounds for top MNCs and startups.",
    topics: [
      "Linear and circular seating arrangements",
      "Syllogisms and Venn diagram deductions",
      "Blood relations and direction sense puzzles",
      "Coding-decoding and series completion logic"
    ],
    focus: [
      "Solving high-difficulty puzzles within 90 seconds",
      "Identifying and skipping time-trap questions during tests",
      "Drawing efficient diagrams to represent groupings"
    ]
  },
  "june-analyst": {
    why: "Establishing the database and ETL foundation needed to handle massive raw cricket data streams for the IPL project.",
    topics: [
      "Complex Joins, Window functions, and Subqueries",
      "Power Query M language and data ingestion techniques",
      "Relational databases, Normal forms (1NF to 3NF), and Schema design"
    ],
    focus: [
      "Optimizing slow-running nested queries in large datasets",
      "Handling Null values and duplicate rows in Power Query",
      "Designing entity-relationship diagrams for cricket matches"
    ]
  },
  "june-project": {
    why: "Hands-on application of data manipulation to build a comprehensive analytics dashboard mapping IPL match details.",
    topics: [
      "Ingesting historical ball-by-ball cricket datasets",
      "Feature engineering (strike rates, economy rates, partnership stats)",
      "Interactive dashboards illustrating team performance vs player matchups"
    ],
    focus: [
      "Handling structural shifts in tournament formats across seasons",
      "Designing intuitive KPIs for non-technical stakeholders",
      "Deploying dashboard on sharing platforms for recruiter view"
    ]
  },
  "june-dsa": {
    why: "Building the rigorous mathematical logic and execution efficiency metrics needed for all subsequent DSA tracks.",
    topics: [
      "Big-O, Big-Theta, and Big-Omega asymptotic notations",
      "Time and space complexity of iterative loops and recursion",
      "Number theory, GCD (Euclidean algorithm), prime factorization, and modular arithmetic"
    ],
    focus: [
      "Avoiding Sieve of Eratosthenes memory overheads in large ranges",
      "Accurately analyzing nested loop bounds under changing variables",
      "Mastering bit-wise math operations for basic calculations"
    ]
  },
  "june-fullstack": {
    why: "Mastering modern web structure and presentation before moving to deep logical client-side rendering.",
    topics: [
      "Semantic HTML5 layouts and accessibility guidelines (ARIA)",
      "CSS flexbox, grid layouts, and custom properties (variables)",
      "Responsive web design, media queries, and viewport adaptation"
    ],
    focus: [
      "Eliminating cross-browser rendering inconsistencies",
      "Structuring clean, reusable, and modular CSS rules",
      "Ensuring mobile-first responsiveness on low-end devices"
    ]
  },
  "june-cscore": {
    why: "Understanding the object-oriented paradigm required for modern software architecture and system design interviews.",
    topics: [
      "Encapsulation, inheritance, polymorphism, and abstraction principles",
      "Classes, objects, constructors, destructors, and access modifiers",
      "Interfaces, abstract classes, and method overriding vs overloading"
    ],
    focus: [
      "Differentiating deep copy vs shallow copy operations in memory",
      "Resolving the diamond problem inheritance ambiguity",
      "Applying SOLID principles to simple class hierarchies"
    ]
  },
  "june-personal": {
    why: "Developing a sharp editorial eye for trending narrative arcs and creative structures by auditing successful media formats.",
    topics: [
      "Deconstructing viral YouTube hooks and script pacing structures",
      "Auditing brand identity campaigns and visual storytelling hooks",
      "Tracking creator monetization frameworks and audience engagement trends"
    ],
    focus: [
      "Translating abstract artistic observations into structured outlines",
      "Maintaining analytical notes instead of passive consumption",
      "Identifying target audience segments for creative outputs"
    ]
  },

  "july-aptitude": {
    why: "Sharpening arithmetic and algebraic skills heavily tested in screening rounds of banking and consulting roles.",
    topics: [
      "Ratio and proportion, percentages, and profit & loss",
      "Time, speed, distance, and work-rate equations",
      "Simple & compound interest formulas and mixture calculations"
    ],
    focus: [
      "Learning short-cut formulas vs analytical derivation",
      "Avoiding calculation fatigue during multi-step word problems",
      "Approximation strategies for multiple-choice options"
    ]
  },
  "july-analyst": {
    why: "Transitioning from structured SQL querying to programmatic data cleaning and statistical visualization.",
    topics: [
      "Pandas DataFrame manipulation and grouping functions",
      "Data cleaning (imputing missing values, outlier detection)",
      "Visualizing distributions, correlations, and heatmaps with Seaborn",
      "Designing customer health and engagement KPIs"
    ],
    focus: [
      "Avoiding SettingWithCopyWarning in pandas operations",
      "Ensuring color choices in graphs represent data neutrally",
      "Documenting data preparation pipelines cleanly"
    ]
  },
  "july-project": {
    why: "Expanding analytics workflows to overseas league datasets (Bangladesh Premier League) to prove model generalization.",
    topics: [
      "Web scraping Cricinfo or importing API datasets",
      "Analyzing spin-vs-pace metrics on subcontinental pitches",
      "Building predictive models for second-innings chases"
    ],
    focus: [
      "Cleaning dirty or missing foreign league statistics",
      "Tuning models for low-scoring match behaviors",
      "Documenting insights in GitHub readme files"
    ]
  },
  "july-dsa": {
    why: "Mastering the foundational memory layouts and fast lookup structures crucial for 60% of all screening code challenges.",
    topics: [
      "Dynamic arrays, two-pointer techniques, and sliding window checks",
      "Hash maps, collision resolution, and frequency count hashing",
      "String manipulation, pattern matching, and rotation analysis",
      "2D matrix operations, traversal (spiral, diagonal), and transformations"
    ],
    focus: [
      "Handling index boundary errors and edge cases (empty arrays)",
      "Avoiding high space overheads in hash maps",
      "Optimizing string copy operations to save memory"
    ]
  },
  "july-fullstack": {
    why: "Learning the core programming language of the web, focusing on asynchronous paradigms and DOM interactions.",
    topics: [
      "Closures, scopes, lexical binding, and 'this' context",
      "Promises, async/await, error handling, and event loop architecture",
      "DOM manipulation, event delegation, bubbling, and capturing"
    ],
    focus: [
      "Handling memory leaks in event listeners",
      "Avoiding callback hell through async chaining",
      "Debugging asynchronous execution order issues"
    ]
  },
  "july-cscore": {
    why: "Learning how database engines organize, cache, retrieve, and lock data to support high concurrent transactions.",
    topics: [
      "ACID properties, transaction states, and isolation levels",
      "Indexing mechanics (B-Trees, B+ Trees, hashing)",
      "Concurrency control (locks, timestamps, deadlocks) and recovery schemes"
    ],
    focus: [
      "Differentiating clustered vs non-clustered indexing",
      "Identifying and resolving deadlock conditions",
      "Writing queries that hit indexes instead of full tables"
    ]
  },
  "july-personal": {
    why: "Honing written expression and storytelling structure by publishing long-form written pieces regularly.",
    topics: [
      "Drafting technical analyses and descriptive product case studies",
      "Structuring logical essay outlines with engaging hooks",
      "Polishing rhythm, pacing, and emotional resonance in creative copy"
    ],
    focus: [
      "Overcoming blank-page anxiety with structured drafts",
      "Maintaining a consistent writing schedule",
      "Developing a unique voice while keeping language concise"
    ]
  },

  "august-aptitude": {
    why: "Ensuring strong communication and reading comprehension capabilities needed for leadership and global client calls.",
    topics: [
      "Reading comprehension and main-idea extraction",
      "Sentence correction, grammar rules, and syntax structures",
      "Para-jumbles and contextual vocabulary application"
    ],
    focus: [
      "Answering ambiguous comprehension questions objectively",
      "Improving reading speed under tight time limits",
      "Recognizing subtle grammatical errors (modifier placement)"
    ]
  },
  "august-analyst": {
    why: "Creating production-ready data pipelines and customer journey reports utilizing advanced business intelligence features.",
    topics: [
      "Data modeling (star schemas, relationships, cardinality)",
      "DAX expressions (CALCULATE, FILTER, time intelligence functions)",
      "Funnel analysis (conversion tracking) and user retention cohorts"
    ],
    focus: [
      "Avoiding recursive relationships in data models",
      "Optimizing heavy DAX formulas to prevent report slowdowns",
      "Designing clean dashboards that highlight key actionable insights"
    ]
  },
  "august-project": {
    why: "Building an end-to-end simulated transaction system to master user flows, carts, and analytics tracking.",
    topics: [
      "Designing database schema for products, orders, and users",
      "Implementing cart state management and checkout logic",
      "Tracking conversion rates, drop-offs, and popular product categories"
    ],
    focus: [
      "Ensuring secure payment flow mocks and clean transaction states",
      "Handling stock counts concurrently to prevent double-selling",
      "Structuring database relationships for efficient order history queries"
    ]
  },
  "august-dsa": {
    why: "Mastering non-linear references and hierarchical data structures required for product ranking and traversal tasks.",
    topics: [
      "Singly, doubly, and circular linked lists with pointer swaps",
      "Stacks & Queues (implementations, dynamic resizing, applications)",
      "Binary Trees, Binary Search Trees, traversals (BFS, DFS)",
      "Heaps (Min/Max heaps, heap sort) and Graph representation (adjacency lists)"
    ],
    focus: [
      "Handling null pointer exceptions in list operations",
      "Recognizing DFS vs BFS tradeoffs in hierarchical structures",
      "Optimizing heap index calculations manually"
    ]
  },
  "august-fullstack": {
    why: "Utilizing component-based architectures to build modular, state-driven, and lightning-fast user interfaces.",
    topics: [
      "React functional components, JSX syntax, and props",
      "State hook (useState), side effects (useEffect), and custom hooks",
      "Component lifecycle, virtual DOM reconciliation, and list keys"
    ],
    focus: [
      "Preventing unnecessary component re-renders",
      "Managing complex state across deeply nested components",
      "Handling component clean-up in useEffect hooks"
    ]
  },
  "august-cscore": {
    why: "Understanding processor scheduling, memory allocation, and concurrency controls at the operating system level.",
    topics: [
      "Process scheduling algorithms (FIFO, SJF, Round Robin)",
      "Memory management (paging, segmentation, virtual memory, thrashing)",
      "Process synchronization, semaphores, mutexes, and critical sections"
    ],
    focus: [
      "Calculating CPU scheduling execution times",
      "Avoiding page fault bottlenecks in virtual memory systems",
      "Resolving race conditions in concurrent process scenarios"
    ]
  },
  "august-personal": {
    why: "Learning the business and distribution channels behind creative content to design viable marketing plans.",
    topics: [
      "Keyword research and mapping search intents",
      "Building content calendars with consistent thematic hooks",
      "Analyzing creator distribution channels and audience acquisition loops"
    ],
    focus: [
      "Balancing SEO keyword density with genuine reader value",
      "Formulating realistic distribution schedules for busy timelines",
      "Mapping analytics to qualitative storytelling improvements"
    ]
  },

  "september-aptitude": {
    why: "Polishing advanced deduction and puzzle solving skills for final-stage behavioral and logic rounds.",
    topics: [
      "Matrix grids, ordering puzzles, and schedule assignments",
      "Input-output routing, machine logic, and sequence operations",
      "Logical deductions, assumptions, and critical reasoning paths"
    ],
    focus: [
      "Breaking down highly complex, multi-variable logic puzzles",
      "Managing time limits on lengthy case-style setups",
      "Eliminating incorrect answer choices systematically"
    ]
  },
  "september-analyst": {
    why: "Optimizing marketing funnels and tracking organic growth metrics to drive customer acquisition loops.",
    topics: [
      "Google Analytics 4 event tracking and custom tag triggers",
      "Technical SEO architecture (sitemaps, indexing, page speeds)",
      "A/B test design (sample sizes, significance, p-values)",
      "Calculating Customer Lifetime Value (LTV) and Acquisition Cost (CAC)"
    ],
    focus: [
      "Configuring GA4 cross-domain tracking accurately",
      "Interpreting A/B test results without p-hacking",
      "Aligning marketing budgets based on LTV/CAC ratios"
    ]
  },
  "september-project": {
    why: "Simulating a subscription media delivery platform to master video cataloging, recommendations, and user behavior analytics.",
    topics: [
      "Schema design for movies, genres, profiles, and watches",
      "Implementing recommendation algorithms based on viewing patterns",
      "Designing admin dashboards for media uploads and subscription counts"
    ],
    focus: [
      "Optimizing media listing page load speeds",
      "Structuring relational tables for rapid profile switching",
      "Implementing mock subscriptions and user session updates"
    ]
  },
  "september-dsa": {
    why: "Transitioning from static data containers to optimized runtime execution techniques for data filtering.",
    topics: [
      "Binary search variants (upper/lower bounds, rotated arrays)",
      "Sorting implementations (Quick Sort, Merge Sort, Radix Sort)",
      "Two-pointer traversals (pairs, triplets, partition checks)",
      "Sliding window applications (substring tracking, subarray sums)"
    ],
    focus: [
      "Handling infinite loops in custom binary search code",
      "Managing recursion stack limit flags in quicksort routines",
      "Designing sliding window boundary conditions without off-by-one bugs"
    ]
  },
  "september-fullstack": {
    why: "Building robust server-side APIs, routing systems, and database handlers to connect client actions to persist data.",
    topics: [
      "Node.js event-driven architecture and file system actions",
      "Express framework routing, middleware, and request/response flow",
      "MongoDB schema design, index configurations, and Mongoose aggregation pipelines"
    ],
    focus: [
      "Implementing secure JSON Web Token (JWT) user authorization",
      "Handling API error states and edge cases cleanly",
      "Designing scalable database document schemas"
    ]
  },
  "september-cscore": {
    why: "Understanding network protocol stacks, routing architectures, and data transmission rules across the internet.",
    topics: [
      "OSI reference model layers and TCP/IP stack protocols",
      "IP addressing (subnets, routing algorithms, DNS resolution)",
      "Transport protocols (TCP handshakes, congestion control vs UDP)"
    ],
    focus: [
      "Analyzing packet routing through subnet boundaries",
      "Diagnosing packet drop scenarios in heavy traffic models",
      "Choosing UDP vs TCP based on app latency needs"
    ]
  },
  "september-personal": {
    why: "Applying structural storytelling concepts to physical multimedia asset generation for personal branding.",
    topics: [
      "Scriptwriting for short-form video formats (retention hooks)",
      "Audio editing and recording setup for podcasts",
      "Designing visual assets and thumb-stopping thumbnail layouts"
    ],
    focus: [
      "Focusing on core hooks in the first 3 seconds of scripts",
      "Maintaining high audio quality under limited recording setups",
      "Structuring video overlays to match vocal pacing"
    ]
  },

  "october-aptitude": {
    why: "Mastering probability, statistics, and advanced equations for data-centric screening profiles.",
    topics: [
      "Permutations, combinations, and probability models",
      "Algebra, quadratic equations, and coordinate geometry basics",
      "Data interpretation (line charts, radar graphs, caselet tables)"
    ],
    focus: [
      "Calculating complex probability models quickly",
      "Drawing clean insights from dense data tables",
      "Applying short cuts to algebraic expressions"
    ]
  },
  "october-analyst": {
    why: "Harnessing LLM APIs and workflow automation tools to scale data aggregation and analysis workflows.",
    topics: [
      "Few-shot prompt design, system guidelines, and model temperature",
      "Natural Language Processing (tokenization, sentiment analysis, NER)",
      "API integrations (REST, OAuth authentication schemes)",
      "No-code automation workflows using Make and Zapier"
    ],
    focus: [
      "Preventing LLM hallucination in automated data extraction",
      "Handling rate limits and error responses on third-party APIs",
      "Securing API tokens within environment variable vaults"
    ]
  },
  "october-project": {
    why: "Building a localized Telugu literature platform to showcase multilingual search, rich fonts, and character tracking.",
    topics: [
      "Designing character maps and historical timeline databases",
      "Implementing multilingual fuzzy search models for regional scripts",
      "Writing rich metadata trees for poems, authors, and eras"
    ],
    focus: [
      "Ensuring non-Latin font assets load cleanly without layout shifts",
      "Tuning search engines to recognize regional grammar variants",
      "Structuring timeline databases for dynamic navigation"
    ]
  },
  "october-dsa": {
    why: "Mastering dynamic state tracking and memoization strategies required for complex algorithmic optimization.",
    topics: [
      "Prefix sums, difference arrays, and range query models",
      "Recursion trees, backtracking flows, and state transitions",
      "Greedy algorithms (scheduling, optimal choice paths)",
      "Dynamic programming (memoization, tabulation, knapsack, LCS)"
    ],
    focus: [
      "Formulating subproblem transitions for dynamic programming cases",
      "Handling recursive stack limit limits on deep recursion paths",
      "Identifying greedy choice properties before implementation"
    ]
  },
  "october-fullstack": {
    why: "Connecting relational or document store databases to external REST/GraphQL API boundaries securely.",
    topics: [
      "Relational schema design vs NoSQL collections",
      "Query optimization (indexing, aggregate metrics, pagination)",
      "REST API design specifications and Swagger documentation"
    ],
    focus: [
      "Securing API endpoints from SQL/NoSQL injection paths",
      "Implementing rate limiting rules on server interfaces",
      "Normalizing database structures to prevent update anomalies"
    ]
  },
  "october-cscore": {
    why: "Mastering class-level system design patterns to write clean, reusable, and maintainable software architectures.",
    topics: [
      "Creational, structural, and behavioral design patterns",
      "Unified Modeling Language (UML) class diagram design",
      "Coding system simulations (parking lot, movie booking)"
    ],
    focus: [
      "Selecting the appropriate design pattern without over-engineering",
      "Ensuring weak coupling between modular interfaces",
      "Writing testable class structures"
    ]
  },
  "october-personal": {
    why: "Synthesizing individual creative assets into expansive intellectual property structures and storylines.",
    topics: [
      "Authoring lore timelines and multi-character world bibles",
      "Drafting dialogue guides and styling rules for voice actors",
      "Directing content collaboration pipelines across platforms"
    ],
    focus: [
      "Maintaining consistency across massive plot/world timelines",
      "Providing constructive feedback to creative collaborators",
      "Aligning creative releases with clear metrics"
    ]
  },

  "november-aptitude": {
    why: "Finalizing communication skills for recruiter screening and executive interviews.",
    topics: [
      "Critical reasoning (assumptions, strengthening/weakening points)",
      "Advanced comprehension passages with logical abstractions",
      "Sentence completion and logical connectors"
    ],
    focus: [
      "Deciding between two closely matching options objectively",
      "Synthesizing complex paragraphs under limited time frames",
      "Presenting clear communication cues in verbal answers"
    ]
  },
  "november-analyst": {
    why: "Combining statistical forecasts with financial fundamentals to deliver compelling business recommendations.",
    topics: [
      "Linear & logistic regressions, time-series forecasting",
      "Financial statement structure (P&L, Balance Sheet, cash flows)",
      "Data storytelling (pitch decks, business case writing)"
    ],
    focus: [
      "Avoiding overfitting in predictive modeling pipelines",
      "Translating complex statistical models for non-technical leaders",
      "Building realistic business cases using financial ratios"
    ]
  },
  "november-project": {
    why: "Developing a platform to match candidates with job postings based on skills, resume parsing, and role metrics.",
    topics: [
      "Parsing resume files and extracting candidate skill sets",
      "Implementing keyword scoring algorithms for ATS matching",
      "Building dashboards for hiring managers to filter profiles"
    ],
    focus: [
      "Handling unstructured PDF/text data formats during parsing",
      "Tuning matching algorithms to avoid bias or search blindspots",
      "Securing candidate profile records and search queries"
    ]
  },
  "november-dsa": {
    why: "Mastering advanced tree structures for rapid search completions and dynamic range queries.",
    topics: [
      "Tries (insert, search, prefix suggestions, autocomplete design)",
      "Segment Trees (range queries, point updates, lazy propagation)",
      "Binary Indexed Trees (Fenwick trees) for prefix sums and updates"
    ],
    focus: [
      "Optimizing memory usage in trie node pointers",
      "Debugging lazy propagation logic in segment trees",
      "Converting indexing coordinates between trees and arrays"
    ]
  },
  "november-fullstack": {
    why: "Consolidating all projects into a single portfolio website to showcase developers' skills to hiring managers.",
    topics: [
      "Creating a unified, responsive developer portfolio layout",
      "Integrating projects via live links, descriptions, and source code",
      "Deploying site on high-speed static networks (Vercel/GitHub pages)"
    ],
    focus: [
      "Optimizing image and asset loading to pass lighthouse tests",
      "Ensuring layout displays correctly on mobile screens",
      "Configuring custom domain records and SSL certificates"
    ]
  },
  "november-cscore": {
    why: "Designing scalable, distributed system architectures capable of handling millions of requests per second.",
    topics: [
      "Load balancing, vertical/horizontal scaling, reverse proxies",
      "Caching strategies (CDN, Redis cache, cache invalidation)",
      "System components (queues, message brokers, distributed storage)"
    ],
    focus: [
      "Handling single-points-of-failure in system designs",
      "Designing partition models to prevent database load bottlenecks",
      "Choosing between SQL and NoSQL based on scale profiles"
    ]
  },
  "november-personal": {
    why: "Presenting personal projects, insights, and portfolios to networks to attract direct inbound job opportunities.",
    topics: [
      "Writing engaging project launch articles for LinkedIn",
      "Sharing coding insights regularly on Twitter",
      "Building an email subscriber base or online community"
    ],
    focus: [
      "Formatting technical posts for maximum readability",
      "Maintaining positive engagement with professional networks",
      "Measuring post analytics to optimize content delivery"
    ]
  },

  "december-aptitude": {
    why: "Simulating timed screening tests to ensure peak speed, accuracy, and confidence.",
    topics: [
      "Full-length mock tests matching major company patterns",
      "Speed-solving strategies for arithmetic and logical reasoning",
      "Time allocation schedules based on section weightings"
    ],
    focus: [
      "Maintaining composure under strict test time limits",
      "Recognizing when to skip questions to maximize scores",
      "Analyzing errors from mock feedback profiles"
    ]
  },
  "december-analyst": {
    why: "Polishing project code, documentation, and dashboards to present a cohesive analyst profile.",
    topics: [
      "Refining dashboard layouts for clean styling",
      "Writing data project readmes containing clear business outcomes",
      "Building a master analyst case study slide deck"
    ],
    focus: [
      "Verifying all data links and public dashboards work",
      "Ensuring code files on GitHub are clean and commented",
      "Simplifying project descriptions for quick recruiter reviews"
    ]
  },
  "december-project": {
    why: "Linking completed projects to target job listings to show specific requirement alignments.",
    topics: [
      "Mapping portfolio features directly to role descriptions",
      "Creating custom project demo videos for hiring teams",
      "Aligning resume highlight lists with actual code features"
    ],
    focus: [
      "Ensuring all links in resumes work perfectly",
      "Refining project descriptions to match ATS keywords",
      "Verifying mobile and web performance on portfolios"
    ]
  },
  "december-dsa": {
    why: "Mastering specialized interview algorithms to solve edge-case and optimization problems during technical interviews.",
    topics: [
      "Bitwise operations (masking, count set bits, single numbers)",
      "Backtracking (Sudoku, N-Queens, subset generation)",
      "Divide & Conquer (closest pair, FFT) and Branch & Bound",
      "Randomized algorithms (Quickselect) and geometric checks"
    ],
    focus: [
      "Handling base recursion cases in complex backtracking logic",
      "Optimizing dynamic bit masks to save computation cycles",
      "Proving time complexities for randomized code blocks"
    ]
  },
  "december-fullstack": {
    why: "Simulating live coding interviews and system design walk-throughs to build confidence.",
    topics: [
      "Mock technical interviews with peer feedback loops",
      "Explaining coding choices clearly while writing solutions",
      "System design whiteboard layouts and explanations"
    ],
    focus: [
      "Keeping clean communication lines open during problems",
      "Answering unexpected programming or system design edge cases",
      "Translating code ideas into working logic under pressure"
    ]
  },
  "december-cscore": {
    why: "Reviewing computer science core topics (OS, DBMS, CN, LLD/HLD) for quick recall during technical reviews.",
    topics: [
      "Core concept cheat sheets and quick reference review lists",
      "Answering standard database, OS, and network queries",
      "Whiteboard class layout exercises"
    ],
    focus: [
      "Explaining core concepts (e.g., paging, TCP) concisely",
      "Avoiding rambling answers to simple technical questions",
      "Synthesizing CS theory into real engineering choices"
    ]
  },
  "december-personal": {
    why: "Structuring creative portfolios to highlight storytelling, scripting, and narrative design skills for creative roles.",
    topics: [
      "Consolidating scripts, videos, and articles into a creative portfolio",
      "Building a presentation deck detailing content campaign metrics",
      "Formulating a personal brand statement for interview introductions"
    ],
    focus: [
      "Aligning creative works with clear, measurable business metrics",
      "Highlighting visual layout and typography choices in portfolios",
      "Presenting creative ideas clearly in professional contexts"
    ]
  },
  "june-embedded": {
    why: "Establishing low-level programming foundations to understand hardware control, direct registers configuration, and memory efficiency.",
    topics: ["Variables, Loops, and Functions in C", "Pointers, Addresses, and Dereferencing", "Structures, Unions, and Custom Types", "Bitwise Operators & Bitmasking"],
    focus: ["Eliminating memory leaks in C dynamic arrays", "Using bitwise shifting to configure hardware registers", "File handling & basic data serialization"]
  },
  "july-embedded": {
    why: "Moving from coding logic to hardware mechanics — mapping CPU instructions, physical gates, and finite state machines.",
    topics: ["Logic Gates, Truth Tables, and K-Maps", "Flip-Flops, Counters, and Registers", "Finite State Machines (FSM) Design", "CPU Architecture & Registers Basics", "Assembly Code Instructions"],
    focus: ["Designing state transitions for logic controllers", "Analyzing cache hierarchies and memory latencies", "Writing basic assembly commands for data movement"]
  },
  "august-embedded": {
    why: "Programming microcontrollers directly to communicate with sensors and peripherals using industry standard buses.",
    topics: ["GPIO Configuration & Digital I/O", "Timers, Counters, and Hardware Interrupts", "Serial Communication: UART, SPI, I2C", "Embedded C Bare-metal Drivers", "Arduino & STM32 Platforms"],
    focus: ["Writing non-blocking delays using timers", "Handling race conditions in Interrupt Service Routines", "Debugging SPI/I2C protocol transactions with logic analyzers"]
  },
  "september-embedded": {
    why: "Sensing physical changes and sending data to the cloud, forming the backbone of edge computing and IoT systems.",
    topics: ["Ultrasonic & Temperature Sensors", "Accelerometers & Gyroscopes", "ESP32 Wi-Fi & Bluetooth Stack", "MQTT & HTTP IoT Protocols", "Data Serialization (JSON/CBOR)"],
    focus: ["Optimizing sensor reading noise with filtering", "Implementing network reconnection logic under weak signals", "Designing battery-efficient low-power sleep modes"]
  },
  "october-embedded": {
    why: "Managing concurrent hardware processes deterministically without crashing using real-time operating systems.",
    topics: ["FreeRTOS Task Scheduling & Priorities", "Queues & Inter-task Communication", "Semaphores & Mutexes for resources", "TCP/IP & UDP Sockets Programming"],
    focus: ["Avoiding priority inversion and deadlocks in RTOS", "Managing dynamic memory allocation in a deterministic way", "Optimizing socket latency for real-time telemetry"]
  },
  "november-embedded": {
    why: "Deep diving into advanced hardware description or robot middleware to finalize specialized embedded skills.",
    topics: ["Verilog HDL & RTL System Design", "FPGA Development & Logic Synthesis", "Robot Operating System (ROS Nodes, Topics, Messages)", "PID Control Loops & Motor Feedback", "OpenCV Image Processing on Edge Boards"],
    focus: ["Solving timing constraints in FPGA RTL designs", "Debugging asynchronous ROS node communication networks", "Configuring PID coefficients to prevent motor overshoot"]
  },
  "december-embedded": {
    why: "Packaging your hardware and firmware projects into a unified, professional systems engineering portfolio.",
    topics: ["System Integration & Cloud Syncing", "Edge Device Security Hardening", "Hardware Schematics & BOM Documentation", "Technical System Interview Readiness"],
    focus: ["Writing comprehensive Git READMEs for hardware builds", "Exhibiting edge-to-cloud telemetry in dashboards", "Simulating system failure cases and recovery logs"]
  },
  "june-cybersecurity": {
    why: "Learning Linux command line mechanics and TCP/IP protocol foundations tested in security entry tests.",
    topics: ["Linux Command Line & File Operations", "Permissions (chmod, chown) & Groups", "TCP/IP Protocols (IP, TCP, UDP)", "DNS, HTTP, HTTPS, and Core Ports", "Wireshark Packet Capture & Filters"],
    focus: ["Setting up Ubuntu virtual machines securely", "Inspecting raw network traffic for cleartext passwords", "Writing network scanning scripts to map active hosts"]
  },
  "july-cybersecurity": {
    why: "Identifying common web application attack vectors to audit code and deploy countermeasures.",
    topics: ["OWASP Top 10 Vulnerabilities", "Cross-Site Scripting (XSS) Mechanics", "SQL Injection (SQLi) Extraction", "Cross-Site Request Forgery (CSRF)", "Broken Authentication & Session Flaws"],
    focus: ["Exploiting and fixing vulnerabilities in DVWA", "Writing structured security audit reports", "Setting up local secure session configurations"]
  },
  "august-cybersecurity": {
    why: "Learning ethical hacking methods to scan, test, and assess system vulnerabilities proactively.",
    topics: ["Passive & Active Reconnaissance", "Network Scanning (Nmap configurations)", "Web Vulnerability Scanning (Nikto, Gobuster)", "Intercepting Proxies (Burp Suite Basics)", "Common Vulnerability Scoring System (CVSS)"],
    focus: ["Compiling structured penetration testing logs", "Solving introductory labs on TryHackMe and HackTheBox", "Mapping vulnerabilities to CVSS severity scores"]
  },
  "september-cybersecurity": {
    why: "Securing operating systems and aggregating logs to monitor active threats and unauthorized access.",
    topics: ["Windows Security & Active Directory", "Linux System Hardening Procedures", "System Logs (Syslog, Event Viewer)", "SIEM Foundations (Splunk/ELK)", "Writing Custom Detection Rules"],
    focus: ["Aggregating Windows/Linux logs into ELK stack", "Detecting simulated brute-force logins using Splunk dashboards", "Hardening default OS configurations against exploits"]
  },
  "october-cybersecurity": {
    why: "Transitioning threat protection to cloud environments by mastering identity management and storage security.",
    topics: ["AWS Identity & Access Management (IAM)", "S3 Bucket Security Policies & ACLs", "Network Security Groups & VPC Firewalls", "Secrets Management & API Key Vaults", "CloudTrail Security Logging"],
    focus: ["Enforcing Principle of Least Privilege in IAM", "Locking down public S3 buckets from exposure", "Auditing network access logs for anomaly detection"]
  },
  "november-cybersecurity": {
    why: "Validating your technical skillset by preparing for standard security certifications and complex projects.",
    topics: ["CompTIA Security+ Exam Domains", "Google Cybersecurity Certification Prep", "Security Operations Center (SOC) Workflows", "Incident Response & Playbook Design"],
    focus: ["Solving mock exams with scores above 85%", "Automating log audits via Python regex filters", "Building a secure network configuration architecture"]
  },
  "december-cybersecurity": {
    why: "Consolidating security audit reports, VM setups, and SIEM dashboards into a credible portfolio.",
    topics: ["Security Operations Portfolio Curation", "SOC Analyst Interview Scenarios", "Explaining Vulns to Non-Technical Staff", "Secure DevOps Integration (DevSecOps)"],
    focus: ["Documenting 3 penetration testing write-ups", "Configuring live threat monitoring panels", "Answering incident response scenarios under pressure"]
  },
  "june-devops": {
    why: "Setting up automated shell scripts on Linux servers to run web services smoothly.",
    topics: ["Linux Server Administration", "Bash Shell Scripting & Variables", "Cron Jobs & Task Automation", "Basic Linux Firewalls (UFW/IPTables)", "Hosting Web Servers (Nginx/Apache)"],
    focus: ["Writing automated log rotation bash scripts", "Deploying static website with HTTPS via Nginx", "Configuring SSH key authentication and disabling passwords"]
  },
  "july-devops": {
    why: "Mastering version control and continuous integration pipelines to automate tests on code updates.",
    topics: ["Git Advanced Branching & Rebase", "GitHub Pull Request & Code Reviews", "CI/CD Concepts & Pipelines", "GitHub Actions Workflows & Syntax", "Artifacts & Cache Optimization"],
    focus: ["Building workflow that runs lint and tests on pull request", "Managing deployment secrets securely in GitHub repos", "Setting up cached dependencies to speed up builds"]
  },
  "august-devops": {
    why: "Packaging apps with all system dependencies into lightweight, isolated containers.",
    topics: ["Docker Engine & Architecture", "Writing Dockerfiles & Base Images", "Multi-stage Builds for smaller size", "Docker Compose Multi-container Orchestration", "Docker Networking & Volumes Storage"],
    focus: ["Containerizing a MERN stack app with database storage", "Reducing production image sizes using alpine variants", "Configuring internal docker-compose secure networks"]
  },
  "september-devops": {
    why: "Provisioning and deploying scalable web services using major cloud infrastructure providers.",
    topics: ["AWS Core Services: EC2, S3, IAM", "Virtual Private Clouds (VPC) & Subnets", "Load Balancers & Auto-Scaling Groups", "CloudWatch Metrics & Billing Alarms", "AWS Cloud Practitioner Prep"],
    focus: ["Configuring high availability server behind ALB", "Managing secure VPC route tables and gateways", "Deploying a REST API server pointing to AWS RDS database"]
  },
  "october-devops": {
    why: "Orchestrating containerized services across clusters of servers for auto-healing and scaling.",
    topics: ["Kubernetes Architecture (Control Plane vs Nodes)", "Pods, RepliSets, and Deployments", "K8s Networking: Services & Ingress Controllers", "ConfigMaps & Secrets Configuration", "Persistent Volumes (PV & PVC)"],
    focus: ["Deploying multi-container app on local minikube", "Setting up ingress routing rule for public API endpoints", "Verifying zero-downtime rolling updates in Kubernetes"]
  },
  "november-devops": {
    why: "Monitoring production servers using metrics and logs to catch errors before users do.",
    topics: ["Prometheus Metrics Collection", "Grafana Dashboards & Visualizations", "Alertmanager Configurations", "Centralized Logging (Loki/Fluentd)", "Site Reliability Engineering (SRE) KPIs"],
    focus: ["Building a Grafana dashboard for CPU/RAM metrics", "Configuring Slack alerts for critical server downtimes", "Tracing slow request paths using server logging metrics"]
  },
  "december-devops": {
    why: "Consolidating CI/CD pipelines, K8s configs, and monitoring stacks into a professional portfolio.",
    topics: ["Infrastructure as Code (IaC) Basics", "DevOps Engineer Technical Interviews", "System Design & Scaling Scenarios", "Portfolio Architecture Write-ups"],
    focus: ["Publishing end-to-end automated deployment repo", "Explaining zero-downtime database migrations", "Answering high-concurrency architecture interview questions"]
  },
  "june-uiux": {
    why: "Developing basic visual design skills (color, typography, spatial layout) using industry-standard tools.",
    topics: ["Figma Interface, Canvas, and Vectors", "Color Theory: Harmonies, contrast, HSL", "Typography: Scale, hierarchy, readability", "Grids, Margins, and Alignment Rules", "Landing Page Component layouts"],
    focus: ["Designing a dark-themed responsive landing page", "Ensuring color choices pass WCAG accessibility rules", "Creating typography stylesheets for web layouts"]
  },
  "july-uiux": {
    why: "Shifting focus to user psychology, structuring data, and map navigation flow.",
    topics: ["Information Architecture & Site Maps", "User Flows & Decision Points", "Low-Fidelity Sketching & Wireframes", "UX Research: Interviews & observations", "Interactive Paper Prototypes"],
    focus: ["Drafting complete site map for a complex portal", "Creating mobile wireframes for a music player app", "Testing user navigation flow against common tasks"]
  },
  "august-uiux": {
    why: "Applying structured system building in Figma to create reusable, interactive design systems.",
    topics: ["Figma Components & Variants", "Auto-Layout Rules for responsive components", "Design System Variables & Styles", "Interactive High-Fidelity Prototyping", "Figma Smart Animate transitions"],
    focus: ["Building dynamic navigation bar with auto-layout", "Designing reusable inputs, buttons, and state cards", "Linking high-fidelity screens for an interactive OTT prototype"]
  },
  "september-uiux": {
    why: "Mapping user behavior, personas, and journeys to make product interfaces intuitive.",
    topics: ["User Personas & Demographic Profiles", "Customer Journey Maps (CJM)", "Empathy Maps & Pain Points", "Usability Testing Methods", "Affinity Diagramming"],
    focus: ["Drafting user journey maps for a job-search portal", "Writing usability test scripts for product validation", "Structuring feedback cards into thematic clusters"]
  },
  "october-uiux": {
    why: "Adapting designs across multiple device sizes, aspect ratios, and touch targets.",
    topics: ["Responsive Layout Grid Rules", "Mobile vs Desktop Interaction Patterns", "Touch Target Guidelines (min 48px)", "Design Handoff Documentation", "Redlining & Specifications for SDEs"],
    focus: ["Designing mobile and desktop versions of a dashboard", "Creating spacing documentation for frontend handoffs", "Structuring asset export folders for developer use"]
  },
  "november-uiux": {
    why: "Curating high-fidelity designs into a professional, cohesive case study portfolio.",
    topics: ["Writing UX Case Studies (Context, Goal, Process)", "Designing Mockups & Interactive Demos", "UI Curation on Behance & Dribbble", "Portfolio Website Structure"],
    focus: ["Structuring Bingoo Musicverse UX case study", "Creating animated mockups showcasing main interactions", "Polishing typography and grids across all pages"]
  },
  "december-uiux": {
    why: "Polishing your design portfolio and practicing design pitches for recruitment presentations.",
    topics: ["Design Portfolio Curation", "UI/UX Presentation Pitching", "Product Design Interview Prep", "Whiteboard Design Challenges"],
    focus: ["Documenting 3 complete case studies", "Presenting design decisions based on research data", "Answering whiteboard product strategy challenges"]
  },
  "june-aiml": {
    why: "Mastering Python and multi-dimensional array operations necessary for machine learning data feeds.",
    topics: ["Python OOP, Classes, Inheritance", "File Operations & Handling Exceptions", "NumPy Arrays, Shapes, Reshaping", "Pandas Data Cleaning & Grouping", "Matplotlib & Seaborn Data Plots"],
    focus: ["Parsing and cleaning messy CSV dataset files", "Running fast matrix operations without nested loops", "Plotting distributions of player strike rates (IPL)"]
  },
  "july-aiml": {
    why: "Translating statistical mathematics into predictive algorithms using supervised learning.",
    topics: ["Mean, Median, Standard Deviation", "Linear Regression (Continuous Outputs)", "Logistic Regression (Binary Outputs)", "K-Nearest Neighbors (KNN Classifier)", "Train-Test Splits & Validation"],
    focus: ["Building house price prediction model from scratch", "Tuning classification thresholds for binary outputs", "Calculating evaluation metrics (MSE, Log Loss)"]
  },
  "august-aiml": {
    why: "Learning non-linear decision tree models and ensemble techniques that win Kaggle tournaments.",
    topics: ["Decision Trees & Information Gain", "Random Forest Ensemble Models", "XGBoost & Boosting Foundations", "Classification Metrics: Precision, Recall", "F1 Score & ROC-AUC curves"],
    focus: ["Predicting customer churn based on history", "Tuning tree depth to prevent model overfitting", "Analyzing feature importance scores of credit risk models"]
  },
  "september-aiml": {
    why: "Deploying trained ML model structures as REST APIs that web apps can query in real time.",
    topics: ["FastAPI Routing & Input Schema", "Pydantic Data Validation Models", "Model Serialization (Joblib/Pickle)", "Render & Vercel Server Deployment", "MLflow Run & Parameter Tracking"],
    focus: ["Creating endpoints to query prediction outputs", "Tracking model parameters and metrics across runs", "Building Docker containers to host prediction engines"]
  },
  "october-aiml": {
    why: "Transitioning to unstructured text data using modern language models and prompt engineering.",
    topics: ["Tokenization & Word Token Vocabularies", "Vector Embeddings & Cosine Similarity", "OpenAI & Gemini API Integrations", "Prompt Engineering: Few-shot, Chain-of-Thought", "Vector Databases & Search Basics"],
    focus: ["Developing PDF text extraction and search", "Designing robust prompt templates for clean JSON outputs", "Interfacing LLM chat history logs with local memory"]
  },
  "november-aiml": {
    why: "Building autonomous systems that combine information retrieval (RAG) with tool-calling capabilities.",
    topics: ["Retrieval-Augmented Generation (RAG)", "Document Chunking & Indexing Pipelines", "Vector Databases (Pinecone/Chroma)", "AI Agent Task Planning & Actions", "Tool Calling & Functions Execution"],
    focus: ["Constructing campus placement chatbot query pipeline", "Building multi-agent task workflow using LangGraph", "Implementing semantic caching to save LLM API costs"]
  },
  "december-aiml": {
    why: "Polishing AI/ML codebases, API servers, and RAG prototypes into a robust portfolio.",
    topics: ["AI Project Curation & GitHub Readmes", "Technical ML System Design Interviews", "AI Ethics & Latency Optimization", "Whiteboard ML Architecture Coding"],
    focus: ["Documenting end-to-end RAG system deployment", "Measuring API request latency under high loads", "Answering deep learning optimization questions"]
  }
};
...
function buildSkillsData() {
  ...
  Object.entries(SM_GRID_DATA).forEach(([monthName, trackData]) => {
    Object.entries(trackData).forEach(([trackKey, skillTitle]) => {
      ...
      const det = SM_GRID_DETAILS[id] || smGetDetails(skillTitle);
      const why = det.why || det.impact || "";
      const pros = det.topics || det.pros || [];
      const cons = det.focus || det.cons || [];
      data[id] = {
        id,
        title: skillTitle,
        track: trackKey,
...
const SM_SKILLS_DATA = buildSkillsData();
const SM_TRACKS_LIST = ['aptitude','analyst','project','dsa','fullstack','cscore','embedded','cybersecurity','devops','uiux','aiml','personal'];

function SkillMapTab({ activeTracks = ['aptitude','analyst','project','dsa','fullstack','cscore','personal'] }) {
  const displayedTracks = SM_TRACKS_LIST.filter(tk => activeTracks.includes(tk));
  const { useState: useSt, useEffect: useEff, useRef } = React;
  const [masteredSkills, setMasteredSkills] = useSt(() => {
    try { const s=localStorage.getItem('mastered_skills_v2'); return s?JSON.parse(s):{}; } catch(e){ return {}; }
  });
  const [activeSkillId, setActiveSkillId] = useSt(null);
  const [drawerOpen, setDrawerOpen] = useSt(false);
  ...
  useEff(() => {
    try { localStorage.setItem('mastered_skills_v2', JSON.stringify(masteredSkills)); } catch(e){}
  }, [masteredSkills]);

  useEff(() => {
    try { localStorage.setItem('compact_grid_v1', compactGrid?'1':'0'); } catch(e){}
  }, [compactGrid]);

  const toggleMastered = (id) => {
    setMasteredSkills(prev => ({ ...prev, [id]: !prev[id] }));
  };

  const openDrawer = (id) => { setActiveSkillId(id); setDrawerOpen(true); };
  const closeDrawer = () => { setActiveSkillId(null); setDrawerOpen(false); };

  const activeSkill = activeSkillId ? SM_SKILLS_DATA[activeSkillId] : null;
  const activeTrack = activeSkill ? SM_TRACK_CONFIG[activeSkill.track] : null;

  const skillsList = Object.values(SM_SKILLS_DATA);
  const total = skillsList.length;
  const completed = skillsList.filter(s => masteredSkills[s.id]).length;
  ...
  const gridTplCols = compactGrid 
    ? `90px repeat(${displayedTracks.length}, minmax(120px,1fr))` 
    : `140px repeat(${displayedTracks.length}, minmax(180px,1fr))`;
  ...
  return (
    <div style={{ background:'#0A0A0F', color:'#E8E8F0', fontFamily:"'Inter',sans-serif", minHeight:'100%' }}>
      <style>{`
        ...
        .sm-skill-card.sm-mastered { background:rgba(46,204,113,0.03); border-color:rgba(46,204,113,0.2); }
        .sm-skill-card.sm-dimmed { opacity:0.12; pointer-events:none; }
        .sm-drawer { position:fixed; top:0; right:0; width:500px; height:100vh; background:#111118; border-left:1px solid #1E1E2E; box-shadow:-10px 0 30px rgba(0,0,0,0.6); transform:translateX(100%); transition:transform 0.3s cubic-bezier(0.16,1,0.3,1); z-index:9999; display:flex; flex-direction:column; overflow:hidden; }
        .sm-drawer.sm-drawer-open { transform:translateX(0); }
        .sm-backdrop { position:fixed; inset:0; background:rgba(5,5,8,0.85); backdrop-filter:blur(4px); opacity:0; visibility:hidden; transition:opacity 0.3s,visibility 0.3s; z-index:9998; }
        .sm-backdrop.sm-backdrop-open { opacity:1; visibility:visible; }
        .sm-filter-btn { background:#111118; border:1px solid #1E1E2E; color:#6B6B8A; padding:6px 12px; border-radius:4px; font-size:11px; font-weight:700; cursor:pointer; font-family:'DM Mono',monospace; transition:all 0.2s; }
        .sm-filter-btn:hover { border-color:#F4A72A; color:#E8E8F0; }
        .sm-filter-btn.sm-filter-active { background:#F4A72A; color:#000; border-color:#F4A72A; }
        .sm-resource-link { display:flex; align-items:center; justify-content:space-between; background:#111118; border:1px solid #1E1E2E; padding:12px 16px; border-radius:6px; text-decoration:none; color:#E8E8F0; font-size:13px; font-weight:600; transition:all 0.2s; }
        .sm-resource-link:hover { border-color:#F4A72A; background:rgba(255,255,255,0.02); transform:translateX(2px); }
        ::-webkit-scrollbar { width:6px; height:6px; }
        ::-webkit-scrollbar-track { background:#0A0A0F; }
        ::-webkit-scrollbar-thumb { background:#1E1E2E; border-radius:3px; }
        ::-webkit-scrollbar-thumb:hover { background:#6B6B8A; }
      `}</style>

      {/* Backdrop */}
      <div className={`sm-backdrop${drawerOpen?' sm-backdrop-open':''}`} onClick={closeDrawer} />

      {/* Controls header */}
      <div style={{ background:'linear-gradient(135deg,#0A0A0F 0%,#12121E 50%,#0A0A0F 100%)', borderBottom:'1px solid #1E1E2E', padding:'16px 24px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:16, flexWrap:'wrap', marginBottom:14 }}>
          ...
          <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid #1E1E2E', borderRadius:8, padding:'10px 16px', minWidth:260 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
              <span style={{ fontSize:11, fontWeight:700, color:'#6B6B8A', textTransform:'uppercase', fontFamily:"'DM Mono',monospace" }}>Mastery Progress</span>
              <span style={{ fontSize:13, fontWeight:800, color:'#2ECC71' }}>{percent}%</span>
            </div>
            <div style={{ background:'rgba(255,255,255,0.05)', borderRadius:3, height:6, overflow:'hidden' }}>
              <div style={{ background:'linear-gradient(90deg,#F4A72A,#2ECC71)', height:'100%', width:percent+'%', transition:'width 0.4s ease' }} />
            </div>
          </div>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
          <div style={{ position:'relative', flex:1, minWidth:220, maxWidth:380 }}>
            <span style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', color:'#6B6B8A', fontSize:13, pointerEvents:'none' }}>🔍</span>
            <input
              type="text"
              placeholder="Search skills..."
              value={searchQuery}
              onChange={e=>setSearchQuery(e.target.value)}
              style={{ width:'100%', background:'#111118', border:'1px solid #1E1E2E', borderRadius:6, color:'#E8E8F0', padding:'9px 14px 9px 36px', fontSize:13, outline:'none', fontFamily:'inherit' }}
            />
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
            <span style={{ fontSize:10, color:'#6B6B8A', fontFamily:"'DM Mono',monospace", textTransform:'uppercase', marginRight:4 }}>Track:</span>
            {[['all','All'], ...Object.entries(SM_TRACK_CONFIG).map(([k,v])=>[k,v.label])].map(([k,label]) => (
              <button key={k} className={`sm-filter-btn${trackFilter===k?' sm-filter-active':''}`} onClick={()=>setTrackFilter(k)}>{label}</button>
            ))}
            <button
              className={`sm-filter-btn${compactGrid?' sm-filter-active':''}`}
              onClick={()=>setCompactGrid(p=>!p)}
              style={{ marginLeft:4 }}
            >Compact: {compactGrid?'On':'Off'}</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ width:'100%', overflowX:'auto', borderRadius:0, background:'rgba(255,255,255,0.005)', borderTop:'none', paddingBottom:8 }}>
        <div style={{ display:'grid', gridTemplateColumns:gridTplCols, minWidth: compactGrid ? (90 + displayedTracks.length * 120) : (140 + displayedTracks.length * 180) }}>
          {/* Header row */}
                ...
                return (
                  <div key={tk} style={{ padding:compactGrid?'4px 6px':'12px', borderBottom:'1px solid #1E1E2E', borderRight:'1px solid #1E1E2E', display:'flex', flexDirection:'column', gap:4, opacity:isDimmed?0.12:1, transition:'opacity 0.2s', justifyContent:'center' }}>
                    {cellSkills.map(skill => {
                      ...
                      const isActive = activeSkillId===skill.id && drawerOpen;
                      return (
                        <div
                          key={skill.id}
                          className={`sm-skill-card${isMastered?' sm-mastered':''}${isActive?' sm-active':''}${!vis?' sm-dimmed':''}`}
      ...
      {/* Skill Detail Drawer */}
      <div className={`sm-drawer${drawerOpen?' sm-drawer-open':''}`}>
        {activeSkill && activeTrack && (
          <>
            <div style={{ padding:'22px 24px 16px', borderBottom:'1px solid #1E1E2E', position:'relative', flexShrink:0 }}>
              <div style={{ display:'flex', gap:8, marginBottom:8 }}>
                <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:4, fontFamily:"'DM Mono',monospace", textTransform:'uppercase', background:`rgba(${activeTrack.rgb},0.1)`, color:activeTrack.color, border:`1px solid rgba(${activeTrack.rgb},0.2)` }}>{activeTrack.label}</span>
                <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:4, fontFamily:"'DM Mono',monospace", textTransform:'uppercase', background:'rgba(244,167,42,0.1)', color:'#F4A72A', border:'1px solid rgba(244,167,42,0.2)' }}>{activeSkill.month}</span>
              </div>
              <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:20, fontWeight:700, color:'#E8E8F0', paddingRight:32, lineHeight:1.3 }}>{activeSkill.title}</div>
              <button onClick={closeDrawer} style={{ background:'none', border:'none', color:'#6B6B8A', fontSize:24, cursor:'pointer', position:'absolute', right:24, top:22, lineHeight:1 }}>×</button>
            </div>
            <div style={{ flex:1, overflowY:'auto', padding:24, display:'flex', flexDirection:'column', gap:20 }}>
              {/* Mastery toggle */}
              <div
                onClick={() => toggleMastered(activeSkillId)}
                style={{ background:'rgba(255,255,255,0.02)', border:`1px solid ${masteredSkills[activeSkillId]?'#2ECC71':'#1E1E2E'}`, borderRadius:8, padding:'13px 16px', display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer', transition:'all 0.2s' }}
              >
                <span style={{ fontSize:13, fontWeight:700, display:'flex', alignItems:'center', gap:10 }}>
                  <span style={{ width:18, height:18, borderRadius:4, border:`2px solid ${masteredSkills[activeSkillId]?'#2ECC71':'#1E1E2E'}`, background:masteredSkills[activeSkillId]?'#2ECC71':'transparent', display:'flex', alignItems:'center', justifyContent:'center', color:'#000', fontSize:11, fontWeight:900, flexShrink:0, transition:'all 0.2s' }}>{masteredSkills[activeSkillId]?'✓':''}</span>
                  Mark as Mastered
                </span>
                <span style={{ fontSize:11, color:'#6B6B8A', fontFamily:"'DM Mono',monospace" }}>{masteredSkills[activeSkillId]?'DONE':'TODO'}</span>
              </div>
              {/* Why */}
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'#6B6B8A', textTransform:'uppercase', letterSpacing:1, marginBottom:10, fontFamily:"'DM Mono',monospace", borderBottom:'1px solid rgba(255,255,255,0.04)', paddingBottom:6 }}>Why This Month</div>
                <p style={{ fontSize:13.5, lineHeight:1.6, color:'#E8E8F0', fontStyle:'italic', borderLeft:'2px solid #F4A72A', paddingLeft:14 }}>{activeSkill.why}</p>
              </div>
              {/* Syllabus and Focus */}
              <div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                  <div>
                    <div style={{ fontSize:11, fontWeight:700, color:'#6B6B8A', textTransform:'uppercase', letterSpacing:1, marginBottom:10, fontFamily:"'DM Mono',monospace", borderBottom:'1px solid rgba(255,255,255,0.04)', paddingBottom:6 }}>What You Need to Learn / Master</div>
                    {(activeSkill.pros||[]).map((p,i)=>(
                      <div key={i} style={{ fontSize:12.5, color:'#A7F3D0', display:'flex', gap:6, alignItems:'flex-start', marginBottom:6 }}>
                        <span style={{ color:'#2ECC71', flexShrink:0 }}>✓</span><span>{p}</span>
                      </div>
                    ))}
                  </div>
                  <div>
                    <div style={{ fontSize:11, fontWeight:700, color:'#6B6B8A', textTransform:'uppercase', letterSpacing:1, marginBottom:10, fontFamily:"'DM Mono',monospace", borderBottom:'1px solid rgba(255,255,255,0.04)', paddingBottom:6 }}>Key Placement / Role Focus</div>
                    {(activeSkill.cons||[]).map((c,i)=>(
                      <div key={i} style={{ fontSize:12.5, color:'#93C5FD', display:'flex', gap:6, alignItems:'flex-start', marginBottom:6 }}>
                        <span style={{ color:'#3B82F6', flexShrink:0 }}>✓</span><span>{c}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'#6B6B8A', textTransform:'uppercase', letterSpacing:1, marginBottom:10, fontFamily:"'DM Mono',monospace", borderBottom:'1px solid rgba(255,255,255,0.04)', paddingBottom:6 }}>ATS Keywords Unlocked</div>
                <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                  {getMatchedKeywords(activeSkill.title).length > 0 ? (
                    getMatchedKeywords(activeSkill.title).map((kw, i) => (
                      <span key={i} style={{ fontSize:10, color:'#34D399', border:'1px solid rgba(52,211,153,0.3)', background:'rgba(52,211,153,0.08)', padding:'4px 8px', borderRadius:4, fontWeight:700 }}>{kw}</span>
                    ))
                  ) : (
                    <span style={{ fontSize:11, color:'#6B6B8A' }}>General conceptual foundation.</span>
                  )}
                </div>
              </div>
              ...
              {/* Roles */}
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'#6B6B8A', textTransform:'uppercase', letterSpacing:1, marginBottom:10, fontFamily:"'DM Mono',monospace", borderBottom:'1px solid rgba(255,255,255,0.04)', paddingBottom:6 }}>Roles This Skill Helps</div>
                <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                  {activeSkill.roles && activeSkill.roles.length>0 ? activeSkill.roles.map((r,i)=>(
                    <span key={i} style={{ fontSize:10, color:r.color, border:`1px solid ${r.color}33`, background:`${r.color}11`, padding:'4px 8px', borderRadius:4, fontWeight:700, textTransform:'uppercase' }}>{r.name}</span>
                  )) : <span style={{ fontSize:11, color:'#6B6B8A' }}>General logical & analytical foundation.</span>}
                </div>
              </div>
            </div>
          </>
...
function Roadmap() {
  ...
  const [activeTracks, setActiveTracks] = useState(() => {
    try {
      const saved = localStorage.getItem("roadmap_active_tracks_v3");
      return saved ? JSON.parse(saved) : ['aptitude','analyst','project','dsa','fullstack','cscore','personal'];
    } catch(e) {
      return ['aptitude','analyst','project','dsa','fullstack','cscore','personal'];
    }
  });

  useEffect(() => {
    try {
      localStorage.setItem("roadmap_active_tracks_v3", JSON.stringify(activeTracks));
    } catch(e) {}
  }, [activeTracks]);

  const toggleTrackKey = (trackKey) => {
    setActiveTracks(prev => {
      if (prev.includes(trackKey)) {
        if (prev.length <= 1) return prev;
        return prev.filter(t => t !== trackKey);
      } else {
  ...
  const activeMonths = React.useMemo(() => {
    ...
    const monthConfigs = [
      {
        month: 1,
        title: "Jun 2026: Foundations & Basics",
        theme: "Basics & Foundations",
        color: BLUE,
        roles: ["Embedded Trainee", "Security Intern", "SOC Intern", "DevOps Intern", "UI Designer", "Data Analyst"],
        baseTasks: [
          { task: "Learn SQL basics (SELECT, JOINs, Group By)", type: "sql", reqTrack: "analyst" },
          { task: "Learn Python Pandas & Matplotlib basics", type: "analytics", reqTrack: "analyst" },
          { task: "Prep IPL Match dataset (2008-2026)", type: "project", reqTrack: "analyst" },
          { task: "DSA: Solve dynamic array & logic puzzles in C/C++", type: "dsa", reqTrack: "embedded" },
          { task: "C/C++: Write command shell/menu projects in C", type: "project", reqTrack: "embedded" },
          { task: "Cyber: Learn Linux command line & network ports basics", type: "cscore", reqTrack: "cybersecurity" },
          { task: "Cyber Project: Set up an Ubuntu VM & simple network scanner", type: "project", reqTrack: "cybersecurity" },
          { task: "DevOps: Script server tasks using Shell/Bash scripting", type: "cscore", reqTrack: "devops" },
          { task: "DevOps Project: Host a landing website on a local Linux server", type: "project", reqTrack: "devops" },
          { task: "UI/UX: Master color theory, typography, layouts in Figma", type: "creative", reqTrack: "uiux" },
          { task: "UI/UX Project: Design a responsive desktop landing page", type: "project", reqTrack: "uiux" },
          { task: "AI/ML: Study OOP, NumPy & Pandas foundations in Python", type: "analytics", reqTrack: "aiml" },
          { task: "AI/ML Project: Build a Student Marks Analyzer tool", type: "project", reqTrack: "aiml" },
          { task: "Aptitude: Learn logical reasoning (Seating, blood relations)", type: "aptitude", reqTrack: "aptitude" },
          { task: "DSA: Learn complexity analysis and Big-O notation", type: "dsa", reqTrack: "dsa" },
          { task: "Full Stack: Learn HTML5 semantics, flexbox, CSS grid", type: "fullstack", reqTrack: "fullstack" },
          { task: "CS Core: Learn Object-Oriented Programming (OOPS)", type: "cscore", reqTrack: "cscore" },
          { task: "Creative: Review successful video formats & write outline", type: "creative", reqTrack: "personal" }
        ]
      },
      {
        month: 2,
        title: "Jul 2026: Advanced Data & Workflows",
        theme: "Advanced Workflows",
        color: PURPLE,
        roles: ["Sports Analyst", "Product Analyst", "Hardware Tester", "Web Security Analyst"],
        baseTasks: [
          { task: "Learn Power BI/Tableau for basic dashboards", type: "analytics", reqTrack: "analyst" },
          { task: "Complete IPL Business & AI Intelligence Layers", type: "project", reqTrack: "analyst" },
          { task: "BPL Analysis: Cross-dataset benchmarking", type: "project", reqTrack: "analyst" },
          { task: "Embedded: Study Digital Logic, K-Maps, FSM, and CPU registers", type: "cscore", reqTrack: "embedded" },
          { task: "Embedded Project: Design a simulated Traffic Light Controller FSM", type: "project", reqTrack: "embedded" },
          { task: "Cyber: Learn OWASP Top 10, XSS, SQLi, and CSRF flaws", type: "cscore", reqTrack: "cybersecurity" },
          { task: "Cyber Practice: Complete PortSwigger Web Security Academy modules", type: "project", reqTrack: "cybersecurity" },
          { task: "DevOps: Learn Git operations, pull requests, and branch workflows", type: "cscore", reqTrack: "devops" },
          { task: "DevOps Project: Configure automated deploy via GitHub Actions", type: "project", reqTrack: "devops" },
          { task: "UI/UX: Study user flows, wireframes, information architecture", type: "creative", reqTrack: "uiux" },
          { task: "UI/UX Project: Design a custom Music Streaming App interface", type: "project", reqTrack: "uiux" },
          { task: "AI/ML: Study arithmetic mean, median, variance & regressions", type: "analytics", reqTrack: "aiml" },
          { task: "AI/ML Project: Build a House Price Prediction model", type: "project", reqTrack: "aiml" },
          { task: "Aptitude: Solve quantitative word problems (Work, distance, rates)", type: "aptitude", reqTrack: "aptitude" },
          { task: "DSA: Solve array traversals, strings, dynamic arrays", type: "dsa", reqTrack: "dsa" },
          { task: "Full Stack: Learn JavaScript async loops, event loop, DOM", type: "fullstack", reqTrack: "fullstack" },
          { task: "CS Core: Learn DBMS transaction isolation and B-trees indexing", type: "cscore", reqTrack: "cscore" },
          { task: "Creative: Draft a 5-minute dialogue-only screenplay in Fountain", type: "creative", reqTrack: "personal" }
        ]
      },
      {
        month: 3,
        title: "Aug 2026: E-Commerce, Frameworks & Scaling",
        theme: "Frameworks & Scaling",
        color: GREEN,
        roles: ["Business Analyst", "Embedded Intern", "SOC Intern", "Docker Engineer"],
        baseTasks: [
          { task: "Master Advanced SQL (CTEs, Window functions) & Excel Modeling", type: "sql", reqTrack: "analyst" },
          { task: "E-Commerce: Seasonal Sales & Pricing Analysis", type: "project", reqTrack: "analyst" },
          { task: "E-Commerce: Ads Impact & Funnel Drops", type: "project", reqTrack: "analyst" },
          { task: "Embedded: Program STM32 bare-metal GPIO registers and interrupts", type: "cscore", reqTrack: "embedded" },
          { task: "Embedded Project: Write an LED/Smart Fan sensor driver", type: "project", reqTrack: "embedded" },
          { task: "Cyber: Learn reconnaissance, enumeration, and vuln scanner tools", type: "cscore", reqTrack: "cybersecurity" },
          { task: "Cyber Practice: Set up vulnerable app (DVWA) & audit local ports", type: "project", reqTrack: "cybersecurity" },
          { task: "DevOps: Learn Docker containerization, images, and networking", type: "cscore", reqTrack: "devops" },
          { task: "DevOps Project: Containerize a MERN Full Stack application", type: "project", reqTrack: "devops" },
          { task: "UI/UX: Master Figma Auto-Layout, Components, and Design Systems", type: "creative", reqTrack: "uiux" },
          { task: "UI/UX Project: Design a premium OTT App video streaming dashboard", type: "project", reqTrack: "uiux" },
          { task: "AI/ML: Study Decision Trees, Random Forests, and XGBoost models", type: "analytics", reqTrack: "aiml" },
          { task: "AI/ML Project: Build a Customer Churn & Credit Risk Analyzer", type: "project", reqTrack: "aiml" },
          { task: "Aptitude: Learn reading comprehension strategies & corrections", type: "aptitude", reqTrack: "aptitude" },
          { task: "DSA: Solve linked lists, stacks, queues, heap operations", type: "dsa", reqTrack: "dsa" },
          { task: "Full Stack: Learn React/Vite layout architectures and hooks", type: "fullstack", reqTrack: "fullstack" },
          { task: "CS Core: Learn OS scheduling processes and thread synchronization", type: "cscore", reqTrack: "cscore" },
          { task: "Creative: Draft push notification copies and cart email sequences", type: "creative", reqTrack: "personal" }
        ]
      },
      {
        month: 4,
        title: "Sep 2026: Cloud, APIs & System Integration",
        theme: "APIs & System Integration",
        color: ACCENT,
        roles: ["Growth Analyst", "Financial Analyst", "IoT Developer", "Cloud Architect"],
        baseTasks: [
          { task: "Learn Cohort Analysis & A/B Testing foundations", type: "analytics", reqTrack: "analyst" },
          { task: "E-Commerce: LTV vs CAC Unit Economics", type: "project", reqTrack: "analyst" },
          { task: "Deploy E-Commerce Dashboard & Whitepaper", type: "portfolio", reqTrack: "analyst" },
          { task: "Embedded: Interfacing Accel/Gyro sensors and ESP32 Wi-Fi stack", type: "cscore", reqTrack: "embedded" },
          { task: "Embedded Project: Build a simulated IoT Smart Weather Station", type: "project", reqTrack: "embedded" },
          { task: "Cyber: Study Windows OS Security, Linux hardening, Logs", type: "cscore", reqTrack: "cybersecurity" },
          { task: "Cyber Practice: Build a central log aggregator using Splunk/ELK", type: "project", reqTrack: "cybersecurity" },
          { task: "DevOps: Study AWS Cloud services (EC2, S3, IAM, VPC networks)", type: "cscore", reqTrack: "devops" },
          { task: "DevOps Project: Deploy scalable API server on AWS EC2 & S3", type: "project", reqTrack: "devops" },
          { task: "UI/UX: Create user personas, user journeys and maps", type: "creative", reqTrack: "uiux" },
          { task: "UI/UX Project: Design a comprehensive user flow for Bingoo portal", type: "project", reqTrack: "uiux" },
          { task: "AI/ML: Learn FastAPI, endpoints, and server hosting pipelines", type: "analytics", reqTrack: "aiml" },
          { task: "AI/ML Project: Build and deploy a Placement Predictor API", type: "project", reqTrack: "aiml" },
          { task: "Aptitude: Learn logical reasoning (Syllogisms, circular seating)", type: "aptitude", reqTrack: "aptitude" },
          { task: "DSA: Solve searching, sorting, two pointers, sliding window", type: "dsa", reqTrack: "dsa" },
          { task: "Full Stack: Learn Node.js, Express, REST APIs, and MongoDB", type: "fullstack", reqTrack: "fullstack" },
          { task: "CS Core: Learn Computer Networks (TCP/IP protocols, subnetting)", type: "cscore", reqTrack: "cscore" },
          { task: "Creative: Script 6 short educational explainer videos", type: "creative", reqTrack: "personal" }
        ]
      },
      {
        month: 5,
        title: "Oct 2026: NLP, AI Agents & Advanced Infrastructure",
        theme: "Gen AI & Advanced Infra",
        color: RED,
        roles: ["Product Analyst", "AI Developer", "Cloud Security Analyst", "Kubernetes Admin"],
        baseTasks: [
          { task: "Study Retention, Churn & Engagement Metrics", type: "analytics", reqTrack: "analyst" },
          { task: "OTT: Genre vs Revenue & Demographics", type: "project", reqTrack: "analyst" },
          { task: "Embedded: Multi-task FreeRTOS task scheduling, queues, semaphores", type: "cscore", reqTrack: "embedded" },
          { task: "Embedded Project: Build a real-time FreeRTOS multi-sensor hub", type: "project", reqTrack: "embedded" },
          { task: "Cyber: AWS IAM, S3 bucket security, security groups configuration", type: "cscore", reqTrack: "cybersecurity" },
          { task: "Cyber Project: Implement secrets management & API keys vaults", type: "project", reqTrack: "cybersecurity" },
          { task: "DevOps: Learn Kubernetes deployments, pods, ingress, services", type: "cscore", reqTrack: "devops" },
          { task: "DevOps Project: Deploy a containerized microservice on Kubernetes", type: "project", reqTrack: "devops" },
          { task: "UI/UX: Study responsive grid layouts, breakpoints, mobile web", type: "creative", reqTrack: "uiux" },
          { task: "UI/UX Project: Design Mobile & Desktop views for Bingoo platform", type: "project", reqTrack: "uiux" },
          { task: "AI/ML: Study tokenization, text embeddings, LLM APIs, prompt engineering", type: "analytics", reqTrack: "aiml" },
          { task: "AI/ML Project: Build notes summarizer & interview prep bots", type: "project", reqTrack: "aiml" },
          { task: "Aptitude: Master quantitative time, speed, distance problems", type: "aptitude", reqTrack: "aptitude" },
          { task: "DSA: Solve recursion, prefix sums, greedy choices", type: "dsa", reqTrack: "dsa" },
          { task: "Full Stack: Integrate databases, token authentication, routing", type: "fullstack", reqTrack: "fullstack" },
          { task: "CS Core: Learn Low-Level Design (LLD class hierarchies)", type: "cscore", reqTrack: "cscore" },
          { task: "Creative: Draft character bibles for a 5-part web series", type: "creative", reqTrack: "personal" }
        ]
      },
      {
        month: 6,
        title: "Nov-Dec 2026: Specialization, Projects & Portfolio Launch",
        theme: "Consolidation & Launch",
        color: BLUE,
        roles: ["Strategy Analyst", "Robotics Intern", "AI Engineer", "DevOps Engineer"],
        baseTasks: [
          { task: "OTT: COVID Impact & Business Model Comparison", type: "project", reqTrack: "analyst" },
          { task: "Complete OTT 12-page Deep Report", type: "creative", reqTrack: "analyst" },
          { task: "Embedded: Build ROS control nodes or OpenCV detection pipeline", type: "project", reqTrack: "embedded" },
          { task: "Cyber Project: Deploy Security Monitoring Splunk Dashboard & Vuln scanner", type: "project", reqTrack: "cybersecurity" },
          { task: "DevOps: Integrate Prometheus & Grafana logging/monitoring metrics", type: "cscore", reqTrack: "devops" },
          { task: "DevOps Project: Deploy end-to-end automated CI/CD release pipeline", type: "project", reqTrack: "devops" },
          { task: "UI/UX Project: Create visual portfolio for Musicverse, CineHub & Student Tribe", type: "project", reqTrack: "uiux" },
          { task: "AI/ML: Master Vector DBs, LangChain/LangGraph, RAG chunking, agents", type: "analytics", reqTrack: "aiml" },
          { task: "AI/ML Project: Build Placement AI Assistant & Bingoo Companion", type: "project", reqTrack: "aiml" },
          { task: "Deploy Final Portfolio featuring all completed tracks codebases", type: "portfolio", reqTrack: "personal" },
          { task: "Mock Interviews & Resume Polishing (Systems + Analyst)", type: "aptitude", reqTrack: "aptitude" },
          { task: "Start applying to 20+ target roles in selected tracks", type: "project", reqTrack: "personal" }
        ]
      }
    ];

    monthConfigs.forEach(mc => {
      const activeTasks = mc.baseTasks.filter(t => !t.reqTrack || activeTracks.includes(t.reqTrack));
      let titleSuffix = "";
      const selectedSpecs = [];
      if (activeTracks.includes("embedded")) selectedSpecs.push("Embedded");
      if (activeTracks.includes("cybersecurity")) selectedSpecs.push("Cyber");
      if (activeTracks.includes("devops")) selectedSpecs.push("DevOps");
      if (activeTracks.includes("uiux")) selectedSpecs.push("UI/UX");
      if (activeTracks.includes("aiml")) selectedSpecs.push("AI/ML");

      if (selectedSpecs.length > 0) {
        titleSuffix = ` (Fused with ${selectedSpecs.join(" & ")})`;
      }

      result.push({
        month: mc.month,
        title: mc.title + titleSuffix,
        theme: mc.theme,
        color: mc.color,
        focusRoles: mc.roles.filter(r => {
          if (r.includes("Embedded") && !activeTracks.includes("embedded")) return false;
          if (r.includes("Hardware") && !activeTracks.includes("embedded")) return false;
          if (r.includes("Security") && !activeTracks.includes("cybersecurity")) return false;
          if (r.includes("SOC") && !activeTracks.includes("cybersecurity")) return false;
          if (r.includes("DevOps") && !activeTracks.includes("devops")) return false;
          if (r.includes("UI") && !activeTracks.includes("uiux")) return false;
          if (r.includes("AI") && !activeTracks.includes("aiml")) return false;
          return true;
        }),
        projects: [
          ...activeTasks.filter(t => t.type === "project").map(t => t.task.split(":")[1]?.trim() || t.task),
        ].slice(0, 3),
        skills: [
          ...new Set(activeTasks.map(t => t.type === "sql" || t.type === "analytics" ? t.task.split(":")[0]?.trim() || t.task : null).filter(Boolean))
        ],
        tasks: activeTasks.map(t => ({ task: t.task, type: t.type }))
      });
    });
  ...
  const allTasksCount = activeMonths.reduce((sum, m) => sum + m.tasks.length, 0);
...
const tabs = [
  ...
  const currentMonthData = activeMonths[selectedMonth];
  ...
  return (
    <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
            <div>
              ...
              {(() => {
                ...
                return (
                  <>
                    <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, letterSpacing: -0.5, fontFamily: "'Space Grotesk', sans-serif" }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${BORDER}`, borderRadius: 12, padding: "8px 12px", maxWidth: 500 }}>
              <div style={{ fontSize: 9, fontWeight: 700, color: "#6B6B8A", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>Custom Specialization Fusions</div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {[
                  { id: "analyst", label: "📊 Analytics", color: "#3B82F6" },
                  { id: "embedded", label: "🔌 Embedded", color: "#06B6D4" },
                  { id: "cybersecurity", label: "🛡️ Cyber", color: "#F43F5E" },
                  { id: "devops", label: "☁️ DevOps", color: "#6366F1" },
                  { id: "uiux", label: "🎨 UI/UX", color: "#D946EF" },
                  { id: "aiml", label: "🎯 AI/ML", color: "#14B8A6" },
                ].map(spec => {
                  const isActive = activeTracks.includes(spec.id);
                  return (
                    <button
                      key={spec.id}
                      onClick={() => toggleTrackKey(spec.id)}
                      style={{
                        background: isActive ? spec.color + "25" : "transparent",
                        color: isActive ? spec.color : "#6B6B8A",
                        border: `1px solid ${isActive ? spec.color : "#1E1E2E"}`,
                        borderRadius: 14,
                        padding: "4px 10px",
                        fontSize: 10,
                        fontWeight: 700,
      ...
      {activeTab === "skillmap" ? (
        <div style={{ width: "100%", background: "#0A0A0F", minHeight: "calc(100vh - 140px)" }}>
          <SkillMapTab activeTracks={activeTracks} />
        </div>
      ) : (
        ...
        {activeTab === "overview" && (
          <div>
            ...
            {/* Dynamic Multi-Track Integration Map */}
            {activeTracks.some(t => ['embedded', 'cybersecurity', 'devops', 'uiux', 'aiml'].includes(t)) && (
              <div style={{
                background: "linear-gradient(135deg, #161e2e, #0b0f19)",
                ...
                <h2 style={{ fontSize: 18, fontWeight: 900, color: ACCENT, marginBottom: 8, marginTop: 0 }}>🔌 Multi-Track System Fusion Guide</h2>
                <p style={{ color: TEXT, fontSize: 13, lineHeight: 1.6, marginBottom: 20 }}>
                  You are combining multiple specialized paths. By targeting conceptual overlaps, you can study these topics in parallel without hitting burnout.
                </p>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
                  <div style={{ background: "rgba(255,255,255,0.02)", padding: 16, borderRadius: 8, border: "1px solid rgba(255,255,255,0.05)" }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: "#FFF", marginBottom: 8 }}>🔄 Fused Overlaps & Synergies</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 11, color: MUTED }}>
                      {activeTracks.includes("embedded") && <div>⚡ <strong style={{ color: TEXT }}>Embedded:</strong> Low-level memory pointers complement DSA math logic. Hardware FSMs map to software systems database schemas.</div>}
                      {activeTracks.includes("cybersecurity") && <div>🛡️ <strong style={{ color: TEXT }}>Cybersecurity:</strong> Linux permission levels and TCP/IP networks form the infrastructure base for software deployment.</div>}
                      {activeTracks.includes("devops") && <div>☁️ <strong style={{ color: TEXT }}>DevOps & Cloud:</strong> Docker containerization and AWS provisioning scale full-stack web servers dynamically.</div>}
                      {activeTracks.includes("uiux") && <div>🎨 <strong style={{ color: TEXT }}>UI/UX Design:</strong> Creating high-fidelity Figma components guides frontend layouts and responsive web views.</div>}
                      {activeTracks.includes("aiml") && <div>🎯 <strong style={{ color: TEXT }}>AI & ML:</strong> NumPy/Pandas data cleanups match SQL analytical queries. RAG databases and API deployments round out your engineering capabilities.</div>}
                    </div>
                  </div>
                  <div style={{ background: "rgba(255,255,255,0.02)", padding: 16, borderRadius: 8, border: "1px solid rgba(255,255,255,0.05)" }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: "#FFF", marginBottom: 8 }}>⏱️ Daily Time Block Strategy (6h Total)</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 11, color: MUTED }}>
                      <div>
                        <strong style={{ color: "#3B82F6" }}>1. Daily Routine Tracks (2 Hours):</strong>
                        <div style={{ paddingLeft: 10, marginTop: 2 }}>• 1 Hour: DSA / Aptitude / Logic puzzles (Using C/C++ or Python)</div>
                        <div style={{ paddingLeft: 10 }}>• 1 Hour: Creative / Storytelling Writing (essential mental break)</div>
                      </div>
                      <div>
                        <strong style={{ color: "#10B981" }}>2. Alternating Deep Work Blocks (4 Hours):</strong>
                        <div style={{ paddingLeft: 10, marginTop: 2 }}>• <strong style={{ color: TEXT }}>Day A (Pure Software):</strong> 2h Full Stack API + 2h Data Analytics dashboard work</div>
                        <div style={{ paddingLeft: 10 }}>• <strong style={{ color: TEXT }}>Day B (Systems & Specialization):</strong> 2h Systems (OS/Logs/Hardening) + 2h Specialization projects (Embedded C/Figma/FastAPI/Wireshark)</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
              {[
                ...
                { id: "stagecontroller", emoji: "🎸💡", title: "Audience Stage Controller", week: "Week 13 (Dec)", layers: "Decibel nodes + DMX light control", output: "ESP32 nodes + WebSockets crowd dashboard", color: "#6366F1" },
                { id: "telemetry", emoji: "🔌", title: "The Smart Telemetry Box", week: "Embedded", layers: "Signal Processing", output: "ESP32/STM32 + IMU sensor + MQTT + Canvas charts", color: "#06B6D4" },
                { id: "threatshield", emoji: "🛡️", title: "The Live OWASP Threat Shield", week: "Cybersecurity", layers: "Log Parsing Agent", output: "Node.js application + python scanner + SIEM dashboard", color: "#F43F5E" },
                { id: "pipeline", emoji: "☁️", title: "Auto-Scaling Multi-Service Pipeline", week: "DevOps", layers: "multi-stage Docker", output: "GitOps pipeline + AWS + Grafana Analytics", color: "#6366F1" },
              ].map(p => (
        ...
        {activeTab === "resume" && (
          <div style={{ marginTop: 8 }}>
            <ResumeATSAnalyzer activeTracks={activeTracks} />
          </div>
        )}
...
function getDynamicSkills(role, monthIndex, activeTracks = ['aptitude','analyst','project','dsa','fullstack','cscore','personal']) {
  const baseMonthSkills = {
    june: ["Logical Reasoning", "Advanced SQL", "Excel Power Query", "Schema Design (Process Mapping)", "Complexity Analysis", "OOPs", "Brand Messaging & Copywriting", "Database normalization", "Relational Databases", "Star Schema", "Binary Search", "Sorting Algorithms", "Blog writing", "Creative Writing"],
    july: ["Quantitative Aptitude", "Python (Pandas/Seaborn)", "EDA & Data Interpretation", "KPI Design (Product Metrics)", "JavaScript (Core & DOM)", "DBMS", "Two Pointers", "Sliding Window", "Recursion", "Query Optimization", "Sports Domain Knowledge (IPL/PKL)", "Statistical Thinking"],
    august: ["Verbal Ability", "Power BI & Data Visualization", "DAX Modeling", "Funnel & Retention Analytics", "React hooks", "Operating Systems", "On-page Optimization"],
    september: ["Google Analytics & GA4", "SEO Architecture & Keyword Research", "A/B Testing", "LTV/CAC Metrics", "HubSpot Marketing Automation", "Node.js", "Express", "MongoDB", "Computer Networks", "Firebase Admin", "Google Ads Search", "Technical SEO", "Customer Acquisition Cost (CAC)", "Social Media Analytics", "Firestore/NoSQL Schema"],
  ...
  const monthKeys = ["june", "july", "august", "september", "october", "november", "december"];
  const dynamicMonthSkills = {};
  
  monthKeys.forEach(mKey => {
    const skills = [...(baseMonthSkills[mKey] || [])];
    const gridMonth = mKey.toUpperCase();
    const gridData = SM_GRID_DATA[gridMonth];
    
    if (gridData) {
      ['embedded', 'cybersecurity', 'devops', 'uiux', 'aiml'].forEach(track => {
        if (activeTracks.includes(track)) {
          const val = gridData[track];
          if (val) {
            const parts = val.split(/·|,|\+|\(|\)/).map(p => p.trim()).filter(p => p.length > 2);
            parts.forEach(p => {
              if (!skills.some(s => s.toLowerCase() === p.toLowerCase())) {
                skills.push(p);
              }
            });
          }
    ...
    dynamicMonthSkills[mKey] = skills;
  });

  let current = [...BASE_SKILLS];
  if (monthIndex >= 1) current = [...current, ...dynamicMonthSkills.june];
  if (monthIndex >= 2) current = [...current, ...dynamicMonthSkills.july];
  if (monthIndex >= 3) current = [...current, ...dynamicMonthSkills.august];
  if (monthIndex >= 4) current = [...current, ...dynamicMonthSkills.september];
  if (monthIndex >= 5) current = [...current, ...dynamicMonthSkills.october];
  if (monthIndex >= 6) current = [...current, ...dynamicMonthSkills.november];
  if (monthIndex >= 7) current = [...current, ...dynamicMonthSkills.december];

  let learning = [];
  if (monthIndex === 0) learning = dynamicMonthSkills.june;
  else if (monthIndex === 1) learning = dynamicMonthSkills.july;
  else if (monthIndex === 2) learning = dynamicMonthSkills.august;
  else if (monthIndex === 3) learning = dynamicMonthSkills.september;
  else if (monthIndex === 4) learning = dynamicMonthSkills.october;
  else if (monthIndex === 5) learning = dynamicMonthSkills.november;
  else if (monthIndex === 6) learning = dynamicMonthSkills.december;

  const currentSet = new Set(current.map(s => s.toLowerCase()));
  const learningSet = new Set(learning.map(s => s.toLowerCase()));

  let allFuture = [];
  if (monthIndex < 1) allFuture = [...allFuture, ...dynamicMonthSkills.june];
  if (monthIndex < 2) allFuture = [...allFuture, ...dynamicMonthSkills.july];
  if (monthIndex < 3) allFuture = [...allFuture, ...dynamicMonthSkills.august];
  if (monthIndex < 4) allFuture = [...allFuture, ...dynamicMonthSkills.september];
  if (monthIndex < 5) allFuture = [...allFuture, ...dynamicMonthSkills.october];
  if (monthIndex < 6) allFuture = [...allFuture, ...dynamicMonthSkills.november];
  if (monthIndex < 7) allFuture = [...allFuture, ...dynamicMonthSkills.december];
...
function getDynamicProjects(role, monthIndex, activeTracks = ['aptitude','analyst','project','dsa','fullstack','cscore','personal']) {
  const list = [];
  ...
  const isEmbedded = role === "Embedded Systems & IoT Engineer";
  const isCyber = role === "Cybersecurity Analyst / SOC";
  const isDevOps = role === "Cloud / DevOps Engineer";
  const isUIUX = role === "UI/UX Designer";
  const isAIML = role === "AI / ML Engineer";
  ...
  const hiremap = {
    ...
    bullets: [
      ...
      isPA 
        ...
        : isDevOps
        ? "Designed CI/CD build scripts in GitHub Actions, containerized the application with Docker, and configured Vercel deployment pipelines"
        : isUIUX
        ? "Designed high-fidelity wireframes, responsive components and interactive layouts in Figma for developer handoff"
        : isCyber
        ? "Audited role-based access control (RBAC) and Firestore security rules to prevent unauthorized data access"
        : isAIML
        ? "Engineered keyword-matching algorithms and TF-IDF profile similarity scores to match students with jobs"
      ...
      isDA || isDBA
        ...
        : isCyber
        ? "Conducted OWASP threat modeling on authentication handlers and patched potential session hijacking vulnerabilities"
        : isAIML
        ? "Connected a RAG-based query assistant using vector embeddings to answer placement guidelines"
        : isEmbedded
        ? "Interfaced a card-reader hardware simulator via custom local API to log student event attendance"
  ...
  const sahitya = {
    ...
    bullets: [
      ...
      isGA || isSEO || isCS || isGrowth || isWriter || isCopywriter
        ...
        : isDevOps
        ? "Configured auto-deployments and optimized static Next.js MDX compilation speeds within the build pipelines"
        : isUIUX
        ? "Created a fully responsive Figma design system for the literary portal, establishing custom typography scales and color themes"
        : isCyber
        ? "Hardened Next.js endpoints against cross-site scripting (XSS) and SQL injection (SQLi) attacks on search forms"
        : isAIML
        ? "Developed a character dialogue matching algorithm using NLP text similarity to index dialogue cards"
        : isEmbedded
        ? "Connected an e-ink dynamic calendar display via custom ESP32 serial commands to showcase today's poem"
  ...
  if (monthIndex === 0) {
    ...
    iplBullets = [
      ...
      isDA || isDBA
        ...
        : isAIML
        ? "Trained ensemble models (Random Forest/XGBoost) to forecast ball-by-ball runs and classify wicket event probabilities"
        : isDevOps
        ? "Dockerized the analytics web dashboard and established a GitHub Actions deployment workflow with automatic tests"
        : isUIUX
        ? "Crafted the dashboard visual language in Figma, detailing grid structures, typography hierarchy, and dark mode rules"
        : isCyber
        ? "Performed security auditing and input validation checks to prevent OWASP injection vulnerabilities in query interfaces"
        : isEmbedded
        ? "Simulated real-time cricket bat sensor telemetry streams using ESP32 firmware and MQTT broker connections"
      ...
      isSA 
        ...
        : isAIML
        ? "Deployed model predictions via a FastAPI endpoint for low-latency predictions integrated with live stream overlays"
  ...
  if (monthIndex === 0) {
    ...
    bplBullets = [
      isPA 
        ...
        : isAIML
        ? "Developed a collaborative filtering recommendation engine to predict venue-band fit coefficients and lineup success"
        : isDevOps
        ? "Implemented automated testing pipelines and resource usage monitors for scheduling algorithm servers"
        : isUIUX
        ? "Designed a responsive 3-screen interactive layout in Figma mapping the venue booking flow"
        : isCyber
        ? "Secured checkout transaction APIs and validated JWT roles to prevent privilege escalation during booking"
        : isEmbedded
        ? "Wrote serial drivers for ESP32-connected venue check-in NFC terminals to verify ticket IDs locally"
  ...
  if (monthIndex < 3) {
    ...
    ecomBullets = [
      isDA || isDBA
        ...
        : isAIML
        ? "Engineered a customer churn prediction model and product recommendation engine using Pandas and Scikit-Learn"
        : isDevOps
        ? "Configured Docker containers and set up a load balancer to scale checkout microservices"
        : isUIUX
        ? "Constructed e-commerce customer journey maps (CJM) and wired high-fidelity checkout UI components in Figma"
        : isCyber
        ? "Conducted a web security audit based on OWASP Top 10 to patch XSS and CSRF flaws in the checkout form"
        : isEmbedded
        ? "Simulated an IoT inventory shelf-weight sensor stream to auto-trigger restocking signals"
  ...
  if (monthIndex < 5) {
    ...
    ottBullets = [
      isDA || isSEO || isCS || isWriter || isCopywriter
        ...
        : isAIML
        ? "Engineered a sentiment mining classifier for review text and built a title recommendation engine using cosine similarity"
        : isDevOps
        ? "Established containerized staging environments and CI/CD pipelines to verify recommendation service builds"
        : isUIUX
        ? "Conducted audience research surveys and wireframed interactive streaming interfaces in Figma"
        : isCyber
        ? "Implemented AWS IAM least-privilege security policies and secure Secrets Manager configurations to protect database credentials"
        : isEmbedded
        ? "Programmed an ESP32-based smart TV infrared remote controller simulator to mimic client usage patterns"
      ...
      isPA || isBA || isFSD || isBackend
        ...
        : isAIML
        ? "Developed a user churn forecasting model using logistic regression and random forest algorithms"
  ...
  let telemetryStatus = "";
  let telemetryBullets = [];
  if (monthIndex === 0) {
    telemetryStatus = "Planned (Starts June 2026)";
    telemetryBullets = [
      "Planned: Access physical IMU data on STM32/ESP32 register levels",
      "Planned: Implement edge signal noise filtering (Kalman filter) and transmit data via MQTT"
    ];
  } else if (monthIndex === 1) {
    telemetryStatus = "In Progress (June 2026)";
    telemetryBullets = [
      "Setting up hardware registers and basic I2C/SPI sensor read loops in C",
      "Analyzing raw accelerometer streams and clock configuration latency"
    ];
  } else if (monthIndex === 2) {
    telemetryStatus = "In Progress (July 2026)";
    telemetryBullets = [
      "Implementing low-footprint Complementary/Kalman filtering algorithms at the hardware edge to eliminate sensor noise",
      "Analyzing gyroscope drift and motor vibration profiles in real-time"
    ];
  } else if (monthIndex === 3) {
    telemetryStatus = "In Progress (August 2026)";
    telemetryBullets = [
      "Developing delta-compression and bit-packing functions to reduce wireless payload sizes",
      "Configuring ESP32 Wi-Fi module connections and establishing secure MQTT clients"
    ];
  } else if (monthIndex === 4) {
    telemetryStatus = "In Progress (September 2026)";
    telemetryBullets = [
      "Orchestrating Nginx and Mosquitto broker gateways to route edge payloads to server hubs",
      "Measuring concurrent connection limits and TLS handshake times"
    ];
  } else if (monthIndex === 5) {
    telemetryStatus = "In Progress (October 2026)";
    telemetryBullets = [
      "Developing event-driven Node.js & Socket.io stream microservices",
      "Configuring high-frequency InfluxDB/TimescaleDB time-series storage partitioned by device"
    ];
  } else if (monthIndex === 6) {
    telemetryStatus = "In Progress (November 2026)";
    telemetryBullets = [
      "Building a React-based telemetry monitoring dashboard with real-time Canvas charts (bat speed, vectors)",
      "Analyzing chart rendering frame rates and throttling state updates to optimize UI performance"
    ];
  } else {
    telemetryStatus = "Completed (December 2026)";
    telemetryBullets = [
      "Successfully built and deployed an Edge-Computing Sports Telemetry Tracker (ESP32/STM32 + IMU + MQTT + Node.js)",
      "Processed physical motion data streams and mapped vector shapes to classify swing speeds using KNN classification models",
      "Live Canvas dashboard displays real-time telemetry analytics with zero-latency streaming sockets"
    ];
  }
  ...
  const telemetry = {
    ...
    tags: ["Embedded C", "MQTT", "ESP32 / STM32", "TimescaleDB", "Real-Time Sockets"],
    bullets: telemetryBullets,
  ...
  let threatBullets = [];
  if (monthIndex === 0) {
    threatStatus = "Planned (Starts June 2026)";
    threatBullets = [
      "Planned: Deploy vulnerable full-stack app featuring classic SQLi, XSS, and CSRF vulnerabilities",
      "Planned: Write automated python attack simulation and intrusion log analysis daemon"
    ];
  } else if (monthIndex === 1) {
    threatStatus = "In Progress (June 2026)";
    threatBullets = [
      "Mapping vulnerability entry points (login forms, cookies, query strings) in Express app skeleton",
      "Configuring web server logging models to log detailed request headers"
    ];
  } else if (monthIndex === 2) {
    threatStatus = "In Progress (July 2026)";
    threatBullets = [
      "Deploying vulnerable application endpoints deliberately exposing classic OWASP vulnerabilities",
      "Developing automated Python attack scripts executing brute-force logins and SQLi payload variations"
    ];
  } else if (monthIndex === 3) {
    threatStatus = "In Progress (August 2026)";
    threatBullets = [
      "Building log-tailing agent using Python regex to tail web server access logs in real-time",
      "Analyzing processing overhead of continuous stream regex matching on web servers"
    ];
  } else if (monthIndex === 4) {
    threatStatus = "In Progress (September 2026)";
    threatBullets = [
      "Developing signature-matching detection engine to classify malicious strings (like SQLi quotes, script tags)",
      "Setting up webhook routing to alert admin panels via Discord and Slack channels"
    ];
  } else if (monthIndex === 5) {
    threatStatus = "In Progress (October 2026)";
    threatBullets = [
      "Ingesting security events into Splunk / ELK Stack (Elasticsearch, Logstash, Kibana) index containers",
      "Mapping indexing performance and configuring retention rollup rules"
    ];
  } else if (monthIndex === 6) {
    threatStatus = "In Progress (November 2026)";
    threatBullets = [
      "Building React-based security monitoring dashboard displaying threat geolocation maps and blocked IPs",
      "Auditing performance of dynamic chart overlays mapping attack vectors"
    ];
  } else {
    threatStatus = "Completed (December 2026)";
    threatBullets = [
      "Built deliberately vulnerable full-stack web application with active automated python attack simulator",
      "Implemented python log-parsing daemon matching OWASP payload signatures and dispatching webhook alerts",
      "Deployed interactive React security dashboard showing attack maps and dynamically triggering Fail2ban firewall blocks"
    ];
  }
  ...
  const threatshield = {
    ...
    title: "The Live OWASP Threat Shield",
    status: threatStatus,
    tags: ["Cybersecurity", "Python Exploit Scripts", "Log Auditing", "SIEM (ELK)", "React Shield UI"],
    bullets: threatBullets,
...
const roleRequirements = {
  ...
  "Embedded Systems & IoT Engineer": {
    color: "#06B6D4",
    must_have: ["C/C++ Programming", "Digital Electronics", "Microcontrollers (Arduino/STM32)", "ESP32 & IoT Sensors", "RTOS (FreeRTOS)"],
    good_to_have: ["Verilog/FPGA", "ROS (Robot Operating System)", "OpenCV (Computer Vision)", "Bare-metal programming"],
    ats_keywords: ["embedded systems", "firmware", "microcontrollers", "C programming", "ESP32", "STM32", "Arduino", "sensors", "RTOS", "FreeRTOS", "IoT", "MQTT", "SPI", "I2C", "UART", "hardware debugging"]
  },
  "Cybersecurity Analyst / SOC": {
    color: "#F43F5E",
    must_have: ["Linux & Networking", "OWASP Top 10", "Vulnerability Assessment", "SIEM (Splunk/ELK)", "Cloud Security (AWS IAM)"],
    good_to_have: ["CompTIA Security+", "Google Cybersecurity", "Penetration Testing", "Wireshark Packet Analysis"],
    ats_keywords: ["cybersecurity", "SOC analyst", "information security", "vulnerability assessment", "Splunk", "ELK", "Linux security", "network security", "OWASP Top 10", "firewalls", "SIEM", "incident response"]
  },
  "Cloud / DevOps Engineer": {
    color: "#6366F1",
    must_have: ["Linux Administration", "Git & CI/CD Pipelines", "Docker Containerization", "AWS Cloud Services", "Kubernetes Orchestration"],
    good_to_have: ["Prometheus & Grafana", "Infrastructure as Code", "Terraform", "Shell scripting"],
    ats_keywords: ["DevOps", "cloud engineer", "AWS", "CI/CD", "Docker", "Kubernetes", "Git", "GitHub Actions", "Prometheus", "Grafana", "Linux server", "automation", "system administration"]
  },
  "UI/UX Designer": {
    color: "#D946EF",
    must_have: ["Figma Basics & Design system", "User flow & wireframing", "Auto-layout & Figma components", "User research & CJM", "Responsive design"],
    good_to_have: ["Design handoffs", "Interactive prototyping", "Typography stylesheet", "WCAG accessibility"],
    ats_keywords: ["UI/UX designer", "product design", "Figma", "wireframing", "prototyping", "user research", "user journey", "design system", "auto-layout", "components", "responsive design", "accessibility"]
  },
  "AI / ML Engineer": {
    color: "#14B8A6",
    must_have: ["Python & Data foundations", "Supervised Learning (Regression/KNN)", "Ensemble Models (Random Forest/XGBoost)", "FastAPI Deployment", "RAG & LLM Agents"],
    good_to_have: ["MLflow Tracking", "Vector Databases", "LangChain/LangGraph", "Fine-tuning"],
    ats_keywords: ["machine learning", "ML engineer", "AI engineer", "Python", "NumPy", "Pandas", "regression", "classification", "random forest", "XGBoost", "FastAPI", "RAG", "LLM", "agentic AI", "Vector DB"]
  },
  "Product Analyst": {
    color: "#8B5CF6",
...
function getMatchedKeywords(skillTitle) {
  if (!skillTitle) return [];
  const allKeywords = new Set();
  Object.values(roleRequirements).forEach(req => {
    if (req.must_have) req.must_have.forEach(k => allKeywords.add(k));
    if (req.good_to_have) req.good_to_have.forEach(k => allKeywords.add(k));
    if (req.ats_keywords) req.ats_keywords.forEach(k => allKeywords.add(k));
  });

  const matched = [];
  const learnedSet = new Set([skillTitle.toLowerCase()]);
  const parts = skillTitle.split(/·|,|\+/).map(p => p.trim().toLowerCase());
  parts.forEach(p => learnedSet.add(p));

  allKeywords.forEach(kw => {
    const sl = kw.toLowerCase();
    const isMatch = [...learnedSet].some(s => {
...
function ResumeATSAnalyzer({ activeTracks = [] }) {
  ...
  const dynamicResume = {
    ...
    projects: getDynamicProjects(activeRole, monthProgress, activeTracks),
    skills: {
  ...
  const atsScore = Math.min(100, Math.round(((haveCount + learningCount * 0.5) / role.must_have.length) * 100));

  const calculateAtsScore = (roleName) => {
    const roleReq = roleRequirements[roleName];
    if (!roleReq) return 0;
    let totalPoints = 0;
    roleReq.must_have.forEach(skill => {
      if (matchSkill(skill, allCurrentSkills)) {
        totalPoints += 1.0;
      } else if (matchSkill(skill, allLearningSkills)) {
        totalPoints += 0.5;
      }
    });
    return Math.min(100, Math.round((totalPoints / roleReq.must_have.length) * 100));
  };
